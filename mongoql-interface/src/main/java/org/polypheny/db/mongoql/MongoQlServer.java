...
public class MongoQlServer extends QueryInterface {
    ...
    public String anyQuery( final Request request, final Response res ) {
        ...
        if ( parsed.getFamily() == Family.DML && mqlProcessor.needsDdlGeneration( parsed, query.database ) ) {
            mqlProcessor.autoGenerateDDL( getTransaction().createStatement(), parsed, query.database );
        }
