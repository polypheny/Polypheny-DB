...
/**
 * Query predicate analyzer. Uses visitor pattern to traverse existing expression and convert it to {@link QueryBuilder}.
 *
 * Major part of this class have been copied from <a href="https://www.dremio.com/">dremio</a> ES adapter (thanks to their team for improving Polypheny-DB-ES integration).
 */
class PredicateAnalyzer {
    ...
    /**
     * Walks the expression tree, attempting to convert the entire tree into an equivalent Elasticsearch query filter. If an error occurs, or if it
     * is determined that the expression cannot be converted, an exception is thrown and an error message logged.
     *
    ...
    private static class Visitor extends RexVisitorImpl<Expression> {
        ...
        /**
         * Process a call which is a binary operation, transforming into an equivalent query expression. Note that the incoming call may be either a simple binary expression, such as {@code foo > 5}, or it may be several simple expressions connected
         ...
         *
         * NOTE: Some combinations of operands are implicitly not supported and will cause an exception to be thrown. For example, we currently do not support comparing a literal to another literal as convention {@code 5 = 5}. Nor do we support
    ...
    static class SimpleQueryExpression extends QueryExpression {
        ...
        @Override
        public QueryExpression notExists() {
            // Even though Lucene doesn't allow a stand alone mustNot boolean query, Elasticsearch handles this problem transparently on its end.
        ...
        public QueryExpression gt( LiteralExpression literal ) {
            Object value = literal.value();
            builder = addFormatIfNecessary( literal, QueryBuilders.rangeQuery( getFieldReference() ).gt( value ) );
            return this;
        }
