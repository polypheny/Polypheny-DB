...
/**
 * Utility class to generate elastic search queries. Most query builders have been copied from ES distribution. The reason we have separate definition is high-level client dependency on core modules (like lucene, netty, XContent etc.) which
...
class QueryBuilders {
    ...
    static class BoolQueryBuilder extends QueryBuilder {
        ...
        private void writeJsonArray( String field, List<QueryBuilder> clauses, JsonGenerator gen ) throws IOException {
            if ( clauses.isEmpty() ) {
                return;
            }
