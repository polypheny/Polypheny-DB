    ...
    public AlgDataType inferReturnType( OperatorBinding opBinding ) {
        ...
        for ( Integer operand : Util.range( opBinding.getOperandCount() ) ) {
            ...
            if ( type instanceof MapPolyType ) {
                builder.add( null, UnnestOperator.MAP_KEY_COLUMN_NAME, null, type.unwrapOrThrow( MapPolyType.class ).getKeyType() );
                builder.add( null, UnnestOperator.MAP_VALUE_COLUMN_NAME, null, type.unwrapOrThrow( MapPolyType.class ).getValueType() );
            } else {
