...
public class CatalogTest {
    ...
    public void testLayout() throws UnknownDatabaseException, UnknownSchemaException, UnknownTableException, UnknownColumnException {
        ...
        long schemaId = catalog.addNamespace( "test_schema", NamespaceType.RELATIONAL, false );
        LogicalNamespace schema = catalog.getSchema( databaseId, "test_schema" );
        ...
        LogicalColumn column = catalog.getColumn( tableId, "test_column" );
    ...
    public void testSchema() throws UnknownSchemaException {
        ...
        for ( String name : names ) {
            ids.add( catalog.addNamespace( name, NamespaceType.RELATIONAL, false ) );
        }
        assertEquals( catalog.getNamespaces( databaseId, null ).stream().map( s -> s.name ).collect( Collectors.toList() ), names );

        // test renaming of schema
        String replacedName = "newName";
        Long id = ids.get( 0 );
        catalog.renameNamespace( id, replacedName );
        names.remove( 0 );
        ...
        catalog.setNamespaceOwner( id, newUserId );
    ...
    public void testTable() throws GenericCatalogException {
        ...
        long schemaId = catalog.addNamespace( "schema1", NamespaceType.RELATIONAL, false );
        ...
        assertEquals(
                names,
                catalog.getTables( null, null ).stream().sorted().map( s -> s.name ).collect( Collectors.toList() ) );
    ...
    public void testColumn() throws GenericCatalogException {
        ...
        LogicalColumn otherColumn = catalog.getColumn( otherColumnId );
    ...
    public void testKey() throws GenericCatalogException {
        ...
        //assertNull( catalog.getLogicalTable( tableId ).primaryKey );
        ...
        LogicalColumn column3 = catalog.getColumn( columnId3 );
