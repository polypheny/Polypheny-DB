...
| `query_lists/access_model_comparison/` | Access-model comparison benchmark queries for Polypheny relational, Polypheny MQL, DuckDB, and Spark.                                              |
...
- `*_polypheny*.sql`: Polypheny relational SQL with explicit adapter-prefixed table names.
...
- `*_query_specification.md`: logical query intent and query groups.
...
run the benchmark clients, and generate the suite summary.
...
| `scripts/plot_generation/`               | Plot generation scripts, including the generic benchmark CSV plot generator.  |
...
- `results/plots/`: generated plot artifacts such as SVG and PDF files.
