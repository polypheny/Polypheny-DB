...
STANDARD_DEVIATION_NOTE = "Bars show mean; whiskers show +/-1 sample SD."
...
def main():
    ...
    for label, path in inputs:
        ...
        means, standard_deviations, run_counts = read_statistics(path, args.phase)
        series.append(
            Series(
                label,
                visual_label,
    ...
    if not output_name.endswith("_std"):
        output_name += "_std"
    for file_format in parse_formats(args.formats):
        output = output_dir / f"{output_name}.{file_format}"
        if file_format == "svg":
            output.write_text(
                render_svg(
                    series,
...
def read_means(path, phase):
    means, _standard_deviations, _run_counts = read_statistics(path, phase)
    return means


def read_statistics(path, phase):
    elapsed_by_query = {}
    ...
    for query_id, values in elapsed_by_query.items():
        if not values:
            continue
        mean, sample_standard_deviation, run_count = sample_statistics(values)
        means[query_id] = mean
        run_counts[query_id] = run_count
        if sample_standard_deviation is not None:
            standard_deviations[query_id] = sample_standard_deviation
    return means, standard_deviations, run_counts


def sample_statistics(values):
    if not values:
        raise ValueError("At least one elapsed value is required.")
    return (
...
def render_svg(series, query_ids, title, query_descriptions, description_wrap_chars, description_max_lines, side_note="", show_standard_deviation=False):
    ...
    for bar in bars(series, query_ids, geometry):
        ...
        tooltip = bar_tooltip(item, query_id, value, show_standard_deviation)
        lines.append(
    ...
    if show_standard_deviation:
        for center, lower_y, upper_y, cap_width in standard_deviation_whiskers(series, query_ids, geometry):
            lines.append(f'<line x1="{center:.2f}" y1="{upper_y:.2f}" x2="{center:.2f}" y2="{lower_y:.2f}" stroke="#222" stroke-width="1.5"/>')
            lines.append(f'<line x1="{center - cap_width / 2:.2f}" y1="{upper_y:.2f}" x2="{center + cap_width / 2:.2f}" y2="{upper_y:.2f}" stroke="#222" stroke-width="1.5"/>')
            lines.append(f'<line x1="{center - cap_width / 2:.2f}" y1="{lower_y:.2f}" x2="{center + cap_width / 2:.2f}" y2="{lower_y:.2f}" stroke="#222" stroke-width="1.5"/>')
    ...
    for index, note in enumerate(plot_notes(side_note, show_standard_deviation)):
...
def write_png(output, series, query_ids, title, query_descriptions, description_wrap_chars, description_max_lines, side_note="", show_standard_deviation=False):
    try:
    ...
    if show_standard_deviation:
        for center, lower_y, upper_y, cap_width in standard_deviation_whiskers(series, query_ids, geometry):
            draw.line((center, upper_y, center, lower_y), fill="#222222", width=2)
            draw.line((center - cap_width / 2, upper_y, center + cap_width / 2, upper_y), fill="#222222", width=2)
            draw.line((center - cap_width / 2, lower_y, center + cap_width / 2, lower_y), fill="#222222", width=2)
...
def write_pdf(output, series, query_ids, title, query_descriptions, description_wrap_chars, description_max_lines, side_note="", show_standard_deviation=False):
    geometry = make_geometry(
        series,
        query_ids,
        query_descriptions,
    ...
    if show_standard_deviation:
        for center, lower_y, upper_y, cap_width in standard_deviation_whiskers(series, query_ids, geometry):
            line(center, upper_y, center, lower_y, "#222222", 1.5)
            line(center - cap_width / 2, upper_y, center + cap_width / 2, upper_y, "#222222", 1.5)
            line(center - cap_width / 2, lower_y, center + cap_width / 2, lower_y, "#222222", 1.5)
...
def make_geometry(series, query_ids, query_descriptions=None, description_wrap_chars=18, description_max_lines=2, show_standard_deviation=False):
    ...
    lower, upper = plot_bounds(series, show_standard_deviation)
...
def standard_deviation_whiskers(series, query_ids, geometry):
    for x, _y, width, _height, item, query_id, value in bars(series, query_ids, geometry):
        standard_deviation = item.standard_deviations.get(query_id)
        if standard_deviation is None:
            continue
        lower_value = max(geometry.lower, value - standard_deviation)
        upper_value = value + standard_deviation
        yield (
            x + width / 2,
            y_for(lower_value, geometry),
            y_for(upper_value, geometry),
            min(12, width * 0.7),
        )


def bar_tooltip(item, query_id, value, show_standard_deviation):
    if not show_standard_deviation:
        return f"{item.label} {query_id}: {value:.3f} s"
    standard_deviation = item.standard_deviations.get(query_id)
    ...
    return (
        f"{item.label} {query_id}: mean {value:.3f} s; "
        f"sample SD {standard_deviation_text}; n={item.run_counts.get(query_id, 0)}"
    )


def plot_notes(side_note, show_standard_deviation):
    notes = parse_side_note(side_note)
    if show_standard_deviation:
        first_line, second_line = STANDARD_DEVIATION_NOTE.split("; ", 1)
        notes.extend((first_line + ";", second_line))
    return notes
...
def plot_bounds(series, show_standard_deviation=False):
    values = []
    for item in series:
        for query_id, value in item.values.items():
            if value > 0:
                values.append(value)
            if not show_standard_deviation:
                continue
            ...
            if standard_deviation is None:
                continue
            lower_value = value - standard_deviation
            if lower_value > 0:
                values.append(lower_value)
            values.append(value + standard_deviation)
    if not values:
