 ...
 */
public final class ParquetRelRules {

    private ParquetRelRules() {
    }


    public static List<AlgOptRule> rules( ParquetRelConvention out ) {
        return rules( out, AlgFactories.LOGICAL_BUILDER );
    }


    public static List<AlgOptRule> rules(ParquetRelConvention out, AlgBuilderFactory factory ) {
        List<AlgOptRule> rules = new ArrayList<>();
        rules.add( new ParquetAlgOptRule( ParquetRelPatternMatchers.attachFilterToJoinUnderCalc( out, factory ) ) );
        rules.add( new ParquetAlgOptRule( ParquetRelPatternMatchers.attachFieldsAndFiltersToScanUnderCalc( out, factory ) ) );
        rules.add( new EnumerableParquetRule( out, factory ) );
        if ( ParquetOptimizationSettings.isOptimizeAggregation() ) {
            rules.add( new ParquetAlgOptRule( ParquetRelPatternMatchers.joinWithScanOnLeftAndScanOnRight( out, factory ) ) );
            rules.add( new ParquetAlgOptRule( ParquetRelPatternMatchers.aggregateOnScan( out, factory ) ) );
            rules.add( new ParquetAlgOptRule( ParquetRelPatternMatchers.aggregateOnCalcScan( out, factory ) ) );
    ...
    public static class EnumerableParquetRule extends ConverterRule {
        ...
        public EnumerableParquetRule(ParquetRelConvention in, AlgBuilderFactory algBuilderFactory ) {
            super(
                    AlgNode.class,
                    alg -> true,
                    in,
