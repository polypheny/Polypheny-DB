 ...
 */
public class ParquetPartitionAwareFilterEvaluator extends ParquetGroupFilterEvaluator {

    private final ParquetSourceFile sourceFile;
    private final List<ParquetColumnBinding> columnBindings;


    public ParquetPartitionAwareFilterEvaluator( MessageType schema, ParquetValueExtractor valueExtractor, ParquetSourceFile sourceFile, List<ParquetColumnBinding> columnBindings ) {
    ...
    protected Boolean evaluateLeaf( Group group, ParquetAdapterFilter<PolyValue> filter ) {
        if ( filter.columnIndex() >= 0 && filter.columnIndex() < columnBindings.size() && columnBindings.get( filter.columnIndex() ).role() == ParquetColumnRole.PARTITION ) {
            return matchesValue( partitionValue( columnBindings.get( filter.columnIndex() ).columnName() ), filter.operator(), filter.value() );
        }
