...
abstract class DruidJsonFilter implements DruidJson {
    ...
    private static DruidJsonFilter toInKindDruidFilter( RexNode e, AlgDataType rowType, DruidQuery druidQuery ) {
        ...
        Pair<String, ExtractionFunction> druidColumn = DruidQuery.toDruidColumn( ((RexCall) e).getOperands().get( 0 ), rowType, druidQuery );
        final String columnName = druidColumn.left;
        final ExtractionFunction extractionFunction = druidColumn.right;
        if ( columnName == null ) {
            return null;
        ...
        switch ( rexNode.getKind() ) {
            ...
            case IS_FALSE:
                ...
                return simpleFilter != null
                        ? new JsonCompositeFilter( Type.NOT, simpleFilter )
                        : simpleFilter;
            case AND:
            case OR:
            case NOT:
