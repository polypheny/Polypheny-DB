...
@BoolSetting(key = "transferVars", displayName = "Overwrite Workflow Variables", pos = 4,
        defaultValue = true,
        shortDescription = "If true, the workflow variables become accessible in the nested workflow.")
// TODO: allow dynamic variables to be transferred
...
public class NestedWorkflowActivity implements VariableWriter, ContextConsumer {
    ...
    public List<TypePreview> previewOutTypes( List<TypePreview> inTypes, SettingsPreview settings ) throws ActivityException {
        ...
        if ( settings.keysPresent( "fromId", "workflowName", "workflowId", "version" ) ) {
            ...
            if ( nestedManager != null ) {
                ...
                log.error( "INSIDE preview out types" );
    ...
    public void execute( List<CheckpointReader> inputs, Settings settings, ExecutionContextImpl ctx, WritableVariableStore writer ) throws Exception {
        ...
        session.execute( transferVars ? ctx.getVariableStore() : null );
    ...
    @Override
    public void accept( UUID activityId, NestedSessionManager nestedManager ) {
