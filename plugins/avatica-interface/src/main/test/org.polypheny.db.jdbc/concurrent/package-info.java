 ...
 *
 * The class <code>ConcurrentCommandGenerator</code> provides a Java API for constructing concurrent JDBC tests: an instance of the class represents a test case, which contains several sequences of SQL commands (abstracted as subclasses of <code>ConcurrentCommand</code>).
 * Each sequence is run in its own thread as a separate jdbc client (ie a separate <code>java.sql.Connection</code>). There are facilties to synchronize these command threads. Only a simple command sequence is supported: no branching, no looping.
 *
 * An alternative is to define a test by writing a test script in <code>mtsql</code> format, as described below. An instance of <code>ConcurrentCommandScript</code> parses and executes a script.</p>
 *
 * <h3>Script Format</h3>
 *
 * <h4>Syntax:</h4>
 ...
 *
 * When a sql command fails, the rest of its section is skipped. However, if the attribute <i>force</i> is true the error is ignored, and the section continues.  <i>force</i> has an independent value in each section.  Within a section it can be toggled using the sql directive <code>!SET FORCE
 * <i>val</i></code>, where <i>val</i> can be <i>true, false, on, off.</i> (This is modelled after sqlline and sqllineClient. Other sqlline <i>!-</i>directives are ignored.)</p>
 *
 ...
 *
 * <ul>
 * <li>Variable Expansion: If VAR is a declared variable, $VAR is replaced by the value of VAR. Quotes are ignored.  $$ expands to $. A variable cannot expand to a mtsql command name.</li>
 * <li>Variable Declaration: Before being used, a script variable must be explicitly declared in the script (or an included script) by a &#64;var command.</li>
 * </ul>
 *
 * <blockquote><pre>
 * &#64;var VAR                  Declares a variable VAR
 * &#64;var VAR1  VAR2 ... VARn  Declares n variables.
 * </pre></blockquote>
 *
 ...
 *
 * Usage: mtsql [-qvg] -u SERVER -d DRIVER [-n USER] [-p PASSWORD] [VAR=VALUE]* SCRIPT [SCRIPT]*<br>
 ...
 *
 * The output from each thread is stored in a temporary file until the test completes. At that point, the files are merged together into a single <code>.log</code> file containing the results of each thread,
 * in the order the threads were defined. The output for the example script looks like:
 *
