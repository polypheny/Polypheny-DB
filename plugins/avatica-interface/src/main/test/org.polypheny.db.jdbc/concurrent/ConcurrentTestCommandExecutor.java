...
/**
 * ConcurrentTestCommandExecutor is a thread that executes a sequence of {@link ConcurrentTestCommand commands} on a JDBC connection.
 */
class ConcurrentTestCommandExecutor extends Thread {
     ...
     * @param commands the sequence of commands to execute -- null elements indicate no-ops
     * @param synchronizer synchronization object (may not be null);
     * @param debugPrintStream if non-null a PrintStream to use for debugging output (may help debugging thread synchronization
     */
    ...
    public void run() {
        ...
        // stepNumber is used to reconstitute the original step numbers passed by the test case author.
        int stepNumber = 0;
    ...
    /**
     * Sets the thread's current JDBC statement. To clear the JDBC statement use {@link #clearStatement()}.
     */
    ...
    /**
     * Clears the thread's current JDBC statement. To set the JDBC statement use {@link #setStatement(Statement)}.
     */
    ...
    /**
     * Checks whether an exception occurred during execution. If this method returns null, the thread's commands all succeeded. If this method returns non-null,
     * see {@link #getFailureLocation()} for details on which command caused the failure.
     */
    ...
    /**
     * Returns location (e.g., command number) for exception returned by {@link #getFailureCause()}.
     */
    ...
    public static class Sync {
        ...
        synchronized void waitForOthers() throws InterruptedException {
            if ( ++numWaiting == numThreads ) {
                ...
                // REVIEW: SZ 6/17/2004: Need a timeout here -- otherwise a test case will hang forever if there's a deadlock.  The question is, how long should the
