...
package org.polypheny.db.languages.sql.pretty;
 ...
 * <td>Whether the WHEN, THEN and ELSE clauses of a CASE expression appear at the start of a new line.</td>
 * <td>false</td>
 * </tr>
 * <tr>
 ...
 * <td>Whether to enclose all expressions in parentheses, even if the operator has high enough precedence that the parentheses are not required.
 * For example, the parentheses are required in the expression <code>(a + b) c</code> because the '*' operator has higher precedence than the '+' operator, and so without the parentheses, the expression would be
 * equivalent to <code>a + (b * c)</code>. The fully-parenthesized expression, <code>((a + b) * c)</code> is unambiguous even if you don't know the precedence of every operator.</td>
 * <td></td>
 * </tr>
 * <tr>
 * <td>{@link #setQuoteAllIdentifiers QuoteAllIdentifiers}</td>
 * <td>Whether to quote all identifiers, even those which would be correct according to the rules of the {@link SqlDialect} if quotation marks were omitted.</td>
 * <td>true</td>
 * </tr>
 * <tr>
 * <td>{@link #setSelectListItemsOnSeparateLines SelectListItemsOnSeparateLines}
 ...
 * <td>Style for formatting sub-queries. Values are: {@link SqlWriter.SubQueryStyle#HYDE Hyde}, {@link SqlWriter.SubQueryStyle#BLACK Black}.</td>
 * <td>{@link SqlWriter.SubQueryStyle#HYDE Hyde}</td>
 * </tr>
 * <tr>
 * <td>{@link #setLineLength LineLength}</td>
 * <td>Set the desired maximum length for lines (to look nice in editors, printouts, etc.).</td>
 * <td>0</td>
 * </tr>
 * </table>
 */
...
public class SqlPrettyWriter implements SqlWriter {
     ...
     */
    private static final Bean DEFAULT_BEAN = new SqlPrettyWriter( AnsiSqlDialect.DEFAULT ).getBean();
     ...
     *
     * @param pw Writer
     * @param omitDefaults Whether to omit properties whose value is the same as the default
     */
     ...
     * @param open String to print at the start of the list; typically "(" or the empty string.
    ...
    public SqlString toSqlString() {
        ImmutableList<Integer> dynamicParameters = this.dynamicParameters == null ? null : this.dynamicParameters.build();
    ...
    /**
     * Implementation of {@link SqlWriter.Frame}.
     */
    protected class FrameImpl implements Frame {
        ...
        FrameImpl( FrameType frameType, String keyword, String open, String close, int extraIndent, boolean newlineAfterOpen, boolean newlineBeforeSep, int sepIndent, boolean newlineAfterSep, boolean newlineBeforeClose, boolean newlineAfterClose ) {
