 ...
 * <li>Execute in pure java (parsing and validation can be done, but expression evaluation is not possible)
 ...
 *
 * <ul>
 * <li>Parsing all of its the syntactic variants.</li>
 * <li>Deriving the type of in all combinations of arguments.
 * <ul>
 * <li>Pay particular attention to nullability. For example, the result of the "+" operator is NOT NULL if and only if both of its arguments are NOT NULL.</li>
 * <li>Also pay attention to precision/scale/length. For example, the maximum length of the "||" operator is the sum of the maximum lengths of its arguments.</li>
 * </ul>
 * </li>
...
public abstract class SqlOperatorBaseTest extends LanguageManagerDependant {
    ...
    // TODO: Change message when Fnl3Fixed to something like Overflow during calculation or cast: PC=0 Code=22003"
    public static final String OUT_OF_RANGE_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like "Division by zero: PC=0 Code=22012"
    public static final String DIVISION_BY_ZERO_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like "String right truncation: PC=0 Code=22001"
    public static final String STRING_TRUNC_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like "Invalid datetime format: PC=0 Code=22007"
    ...
    public void testBetween() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.BETWEEN ), SqlTester.VmName.EXPAND );
    ...
    public void testNotBetween() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_BETWEEN ), VM_EXPAND );
    ...
    public void testCastIntervalToNumeric() {
        ...
        // interval to decimal
        if ( DECIMAL ) {
            tester.checkScalarExact( "cast(INTERVAL '1.29' second(1,2) as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "1.3" );
        ...
        tester.checkScalarExact( "cast(INTERVAL '1' year as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' year - INTERVAL '2' year) as integer)", "INTEGER NOT NULL", "-1" );
        tester.checkScalarExact( "cast(INTERVAL '1' month as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' month - INTERVAL '2' month) as integer)", "INTEGER NOT NULL", "-1" );
        tester.checkScalarExact( "cast(INTERVAL '1' day as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' day - INTERVAL '2' day) as integer)", "INTEGER NOT NULL", "-1" );
        tester.checkScalarExact( "cast(INTERVAL '1' hour as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' hour - INTERVAL '2' hour) as integer)", "INTEGER NOT NULL", "-1" );
        tester.checkScalarExact( "cast(INTERVAL '1' hour as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' minute - INTERVAL '2' minute) as integer)", "INTEGER NOT NULL", "-1" );
        tester.checkScalarExact( "cast(INTERVAL '1' minute as integer)", "INTEGER NOT NULL", "1" );
        tester.checkScalarExact( "cast((INTERVAL '1' second - INTERVAL '2' second) as integer)", "INTEGER NOT NULL", "-1" );
    }
    ...
    public void testCastIntervalToInterval() {
        ...
        tester.checkScalar( "cast(interval '2 5' day to hour as interval hour to second)", "+53:00:00.000000", "INTERVAL HOUR TO SECOND NOT NULL" );
        tester.checkScalar( "cast(interval '2 5' day to hour as interval hour)", "+53", "INTERVAL HOUR NOT NULL" );
        tester.checkScalar( "cast(interval '-29:15' hour to minute as interval day to hour)", "-1 05", "INTERVAL DAY TO HOUR NOT NULL" );
    }
    ...
    public void testCastApproxNumericLimits() {
        ...
        for ( int i = 0; i < NUMERIC_TYPE_NAMES.length; i++ ) {
            ...
            // Convert from literal to type
            checkCastToApproxOkay( MAX_NUMERIC_STRINGS[i], type, Double.parseDouble( MAX_NUMERIC_STRINGS[i] ), isFloat ? 1E32 : 0 );
            ...
            // TODO: The following tests are slightly different depending on whether the java or fennel calc are used. Try to make them the same
    ...
    public void testCastDateTime() {
        ...
        // Note: Casting to time(0) should lose date info and fractional seconds, then casting back to timestamp should initialize to current_date.
        tester.checkScalar(
    ...
    public void testCastStringToDateTime() {
        ...
        if ( Bug.CALCITE_2539_FIXED ) {
            ...
            tester.checkFails(
                    ...
                    true );
            tester.checkFails(
                    "cast('12:34:5' as TIME)",
                    BAD_DATETIME_MESSAGE,
                    true );
            tester.checkFails(
                    "cast('12:3:45' as TIME)",
                    BAD_DATETIME_MESSAGE,
                    true );
            tester.checkFails(
                    "cast('1:23:45' as TIME)",
                    BAD_DATETIME_MESSAGE,
                    true );
        }
        ...
        if ( Bug.CALCITE_2539_FIXED ) {
            ...
            tester.checkFails(
                    "cast('1945-1-24 12:23:34.454' as TIMESTAMP)",
                    BAD_DATETIME_MESSAGE,
                    true );
        }
            ...
            try {
                switch ( timeUnit ) {
                    case Calendar.DAY_OF_MONTH:
                        // Within two minutes of the end of the day. Wait in 10s increments until calendar moves into the next next day.
                    ...
                    case Calendar.HOUR_OF_DAY:
                        // Within two minutes of the top of the hour. Wait in 10s increments until calendar moves into the next next day.
    ...
    public void testCase() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CASE ) );
        ...
        tester.checkString(
                "case 1 when 1 then cast('a' as varchar(1)) when 2 then cast('bcd' as varchar(3)) end",
                "a",
                "VARCHAR(3)" );
        ...
        // Per spec, 'case x when y then ...' translates to 'case when x = y then ...' so nulls do not match. (Unlike Oracle's 'decode(null, null, ...)', by the way.)
        tester.checkString(
    ...
    public void testJdbcFn() {
        ...
        // There follows one test for each function in appendix C of the JDBC 3.0 specification. The test is 'if-false'd out if the function is not implemented or is broken.
        ...
        // REPLACE returns NULL in Oracle but not in Postgres or in Polypheny-DB. When [POLYPHENYDB-815] is implemented and SqlConformance#emptyStringIsNull is enabled, it will return empty string as NULL.
        tester.checkString(
                "{fn REPLACE('ciao', 'ciao', '')}",
                "",
                "VARCHAR(4) NOT NULL" );

        tester.checkString(
                "{fn REPLACE('hello world', 'o', '')}",
                "hell wrld",
                "VARCHAR(11) NOT NULL" );
        ...
        if ( Bug.CALCITE_2539_FIXED ) {
            tester.checkFails(
                    "{fn DAYOFWEEK(DATE '2014-12-10')}",
                    "cannot translate call EXTRACT.*",
                    true );

            tester.checkFails(
                    "{fn DAYOFYEAR(DATE '2014-12-10')}",
                    "cannot translate call EXTRACT.*",
                    true );
        }
        ...
        // Conversion Functions Legacy JDBC style
    ...
    public void testSelect() {
        ...
        // Check return type on scalar sub-query in select list.  Note return type is always nullable even if sub-query select value is NOT NULL. Bug FRG-189 causes this test to fail only in SqlOperatorTest; not in subtypes.
        if ( Bug.FRG189_FIXED || (getClass() != SqlOperatorTest.class) && Bug.TODO_FIXED ) {
            ...
            tester.checkType(
                    " SELECT *,(SELECT * FROM (VALUES(cast('abcd' as varchar(10))))) FROM (VALUES(CAST('abcd' as varchar(10))))",
                    "RecordType(VARCHAR(10) NOT NULL EXPR$0, VARCHAR(10) EXPR$1) NOT NULL" );
            tester.checkType(
                    "SELECT *, (SELECT * FROM (VALUES(TIMESTAMP '2006-01-01 12:00:05'))) FROM (VALUES(TIMESTAMP '2006-01-01 12:00:05'))",
    ...
    public void testLiteralChain() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LITERAL_CHAIN ), VM_EXPAND );
        tester.checkString(
                "'buttered'\n' toast'",
                "buttered toast",
                "CHAR(14) NOT NULL" );
    ...
    public void testComplexLiteral() {
        ...
        tester.check(
                "select 1 * 2 * 3 * x from (select 2 as x)",
                new SqlTests.StringTypeChecker( "INTEGER NOT NULL" ),
                "12",
                0 );

        tester.check(
                "select 1 + 2 + 3 + 4 + x from (select 2 as x)",
                new SqlTests.StringTypeChecker( "INTEGER NOT NULL" ),
                "12",
                0 );
    }
    ...
    @Test
    public void testRow() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ROW ), VM_FENNEL );
    }


    @Test
    public void testAndOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.AND ) );
    ...
    public void testAndOperatorLazy() {
        ...
        // lazy eval returns FALSE; eager eval executes RHS of AND and throws; both are valid
        tester.check(
    ...
    public void testConcatOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CONCAT ) );
    ...
    public void testModOperator() {
        ...
        tester1.setFor( StdOperatorRegistry.get( OperatorName.PERCENT_REMAINDER ) );
    ...
    public void testDivideOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DIVIDE ) );
    ...
    public void testEqualsOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.EQUALS ) );
    ...
    public void testGreaterThanOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.GREATER_THAN ) );
    ...
    public void testIsDistinctFromOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_DISTINCT_FROM ), VM_EXPAND );
    ...
    public void testIsNotDistinctFromOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_DISTINCT_FROM ), VM_EXPAND );
    ...
    public void testGreaterThanOrEqualOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.GREATER_THAN_OR_EQUAL ) );
        tester.checkBoolean( "1>=2", Boolean.FALSE );
    ...
    public void testInOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IN ), VM_EXPAND );
    ...
    public void testNotInOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_IN ), VM_EXPAND );
    ...
    public void testOverlapsOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.OVERLAPS ), VM_EXPAND );
        ...
        tester.checkBoolean( "(time '1:2:3', interval '1' second) overlaps (time '23:59:59', time '1:2:3')", Boolean.TRUE );
        tester.checkBoolean( "(time '1:2:3', interval '1' second) overlaps (time '23:59:59', time '1:2:2')", Boolean.TRUE );
        tester.checkBoolean( "(time '1:2:3', interval '1' second) overlaps (time '23:59:59', interval '2' hour)", Boolean.FALSE );
        ...
        tester.checkBoolean( "(timestamp '1-2-3 4:5:6', timestamp '1-2-3 4:5:6' ) overlaps (timestamp '1-2-3 4:5:6', interval '1 2:3:4.5' day to second)", Boolean.TRUE );
        tester.checkBoolean( "(timestamp '1-2-3 4:5:6', timestamp '1-2-3 4:5:6' ) overlaps (timestamp '2-2-3 4:5:6', interval '1 2:3:4.5' day to second)", Boolean.FALSE );
        tester.checkNull( "(timestamp '1-2-3 4:5:6', cast(null as interval day) ) overlaps (timestamp '1-2-3 4:5:6', interval '1 2:3:4.5' day to second)" );
        tester.checkNull( "(timestamp '1-2-3 4:5:6', timestamp '1-2-3 4:5:6' ) overlaps (cast(null as timestamp), interval '1 2:3:4.5' day to second)" );
    }
    ...
    public void testLessThanOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LESS_THAN ) );
    ...
    public void testLessThanOrEqualOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LESS_THAN_OR_EQUAL ) );
        tester.checkBoolean( "1<=2", Boolean.TRUE );
        tester.checkBoolean( "1<=1", Boolean.TRUE );
        tester.checkBoolean( "-1<=1", Boolean.TRUE );
        tester.checkBoolean( "2<=1", Boolean.FALSE );
    ...
    public void testMinusOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MINUS ) );
    ...
    public void testMinusIntervalOperator() {
        ...
        tester.checkScalar(
                ...
                "11:02:01",
                "TIME(0) NOT NULL" );
        // Per [POLYPHENYDB-1632] Return types of datetime + interval
    ...
    public void testMinusDateOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MINUS_DATE ) );
        tester.checkScalar(
        ...
        tester.checkScalar(
                "timestamp '1969-04-29 0:0:0' + (timestamp '2008-07-15 15:28:00' - timestamp '1969-04-29 0:0:0') day to second / 2",
                "1988-12-06 07:44:00",
                "TIMESTAMP(0) NOT NULL" );

        tester.checkScalar(
                "date '1969-04-29' + (date '2008-07-15' - date '1969-04-29') day / 2",
                "1988-12-06",
                "DATE NOT NULL" );

        tester.checkScalar(
                "time '01:23:44' + (time '15:28:00' - time '01:23:44') hour to second / 2",
                "08:25:52",
                "TIME(0) NOT NULL" );

        if ( Bug.DT1684_FIXED ) {
            tester.checkBoolean( "(date '1969-04-29' + (CURRENT_DATE - date '1969-04-29') day / 2) is not null", Boolean.TRUE );
        }
    ...
    public void testMultiplyOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MULTIPLY ) );
    ...
    public void testNotEqualsOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_EQUALS ) );
        tester.checkBoolean( "1<>1", Boolean.FALSE );
    ...
    public void testOrOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.OR ) );
    ...
    public void testOrOperatorLazy() {
        ...
        // need to evaluate 2nd argument if first evaluates to null, therefore get error
        tester.check(
                "values 1 < cast(null as integer) or sqrt(-4) = -2",
                SqlTests.BOOLEAN_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER,
                new ValueOrExceptionResultChecker( null, INVALID_ARG_FOR_POWER, CODE_2201F ) );

        // Do not need to evaluate 2nd argument if first evaluates to true. In eager evaluation, get error; lazy evaluation returns true; both are valid.
        tester.check(
        ...
        // NULL OR FALSE --> NULL
        // In eager evaluation, get error; lazy evaluation returns NULL; both are valid.
        tester.check(
                "values 1 < cast(null as integer) or sqrt(4) = -2",
                SqlTests.BOOLEAN_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER,
                new ValueOrExceptionResultChecker( null, INVALID_ARG_FOR_POWER, CODE_2201F ) );
    ...
    public void testPlusOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.PLUS ) );
    ...
    public void testDescendingOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DESC ), VM_EXPAND );
    }


    @Test
    public void testIsNotNullOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_NULL ) );
    ...
    public void testIsNullOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NULL ) );
    ...
    public void testIsNotTrueOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_TRUE ) );
    ...
    public void testIsTrueOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_TRUE ) );
    ...
    public void testIsNotFalseOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_FALSE ) );
    ...
    public void testIsFalseOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_FALSE ) );
    ...
    public void testIsNotUnknownOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_UNKNOWN ), VM_EXPAND );
    ...
    public void testIsUnknownOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_UNKNOWN ), VM_EXPAND );
    ...
    public void testIsASetOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_A_SET ), VM_EXPAND );
        tester.checkBoolean( "multiset[1] is a set", Boolean.TRUE );
        tester.checkBoolean( "multiset[1, 1] is a set", Boolean.FALSE );
        tester.checkBoolean( "multiset[cast(null as boolean), cast(null as boolean)] is a set", Boolean.FALSE );
    ...
    public void testIsNotASetOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_A_SET ), VM_EXPAND );
        tester.checkBoolean( "multiset[1] is not a set", Boolean.FALSE );
        tester.checkBoolean( "multiset[1, 1] is not a set", Boolean.TRUE );
        tester.checkBoolean( "multiset[cast(null as boolean), cast(null as boolean)] is not a set", Boolean.TRUE );
    ...
    public void testIntersectOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MULTISET_INTERSECT ), VM_EXPAND );
        tester.checkScalar(
                "multiset[1] multiset intersect multiset[1]",
                "[1]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[2] multiset intersect all multiset[1]",
                "[]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[2] multiset intersect distinct multiset[1]",
                "[]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[1, 1] multiset intersect distinct multiset[1, 1]",
                "[1]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[1, 1] multiset intersect all multiset[1, 1]",
                "[1, 1]",
                "INTEGER NOT NULL MULTISET NOT NULL" );
        ...
        tester.checkScalar(
                "multiset[cast(null as integer), cast(null as integer)] multiset intersect distinct multiset[cast(null as integer)]",
                "[null]",
                "INTEGER MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[cast(null as integer), cast(null as integer)] multiset intersect all multiset[cast(null as integer)]",
                "[null]",
                "INTEGER MULTISET NOT NULL" );
    ...
    public void testExceptOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MULTISET_EXCEPT ), VM_EXPAND );
        tester.checkScalar(
                "multiset[1] multiset except multiset[1]",
                "[]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[1] multiset except distinct multiset[1]",
                "[]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[2] multiset except multiset[1]",
                "[2]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "multiset[1,2,3] multiset except multiset[1]",
                "[2, 3]",
                "INTEGER NOT NULL MULTISET NOT NULL" );

        tester.checkScalar(
                "cardinality(multiset[1,2,3,2] multiset except distinct multiset[1])",
                "2",
                "INTEGER NOT NULL" );

        tester.checkScalar(
                "cardinality(multiset[1,2,3,2] multiset except all multiset[1])",
                "3",
                "INTEGER NOT NULL" );
    ...
    public void testIsEmptyOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_EMPTY ), VM_EXPAND );
    ...
    public void testIsNotEmptyOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.IS_NOT_EMPTY ), VM_EXPAND );
    ...
    public void testExistsOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.EXISTS ), VM_EXPAND );
    }


    @Test
    public void testNotOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT ) );
    ...
    public void testPrefixMinusOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.UNARY_MINUS ) );
        tester.checkFails(
                "'a' + ^- 'b'^ + 'c'",
    ...
    public void testPrefixPlusOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.UNARY_PLUS ), VM_EXPAND );
        tester.checkScalarExact( "+1", "1" );
    ...
    public void testExplicitTableOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.EXPLICIT_TABLE ), VM_EXPAND );
    }


    @Test
    public void testValuesOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.VALUES ), VM_EXPAND );
        tester.check(
    ...
    public void testNotLikeOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_LIKE ), VM_EXPAND );
    ...
    public void testLikeEscape() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LIKE ) );
    ...
    public void testNotSimilarToOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_SIMILAR_TO ), VM_EXPAND );
    ...
    public void testSimilarToOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SIMILAR_TO ) );
        ...
        // The following two tests throws exception(They probably should). "Dangling meta character '*' near index 2"
        ...
        // all the following tests wrong results due to missing functionality or defect (FRG-375, 377).

        if ( Bug.FRG375_FIXED ) {
            tester.checkBoolean( "'cd' similar to '[a-e^c]d' ", Boolean.FALSE ); // FRG-375
        }

        // following tests use regular character set identifiers. Not implemented yet. FRG-377.
        if ( Bug.FRG377_FIXED ) {
            tester.checkBoolean(
                    "'y' similar to '[:ALPHA:]*'",
                    Boolean.TRUE );
            ...
            tester.checkBoolean(
                    "'YD  3223' similar to '[:UPPER:]{2}||[:WHITESPACE:]*[:DIGIT:]{4}'",
                    Boolean.TRUE );
    ...
    public void testEscapeOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ESCAPE ), VM_EXPAND );
    }


    @Test
    public void testConvertFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CONVERT ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testTranslateFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TRANSLATE ), VM_FENNEL, VM_JAVA );
    }
    ...
    */
    ...
    public void testOverlayFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.OVERLAY ) );
        tester.checkString(
        ...
        if ( enable ) {
            tester.checkString(
                    "overlay(cast('ABCdef' as char(10)) placing cast('abc' as char(5)) from 1 for 2)",
                    "abc  Cdef    ",
                    "VARCHAR(15) NOT NULL" );
        }
    ...
    public void testPositionFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.POSITION ) );
    ...
    public void testReplaceFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.REPLACE ) );
    ...
    public void testCharLengthFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CHAR_LENGTH ) );
    ...
    public void testCharacterLengthFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CHARACTER_LENGTH ) );
    ...
    public void testUpperFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.UPPER ) );
    ...
    public void testJsonExists() {
        ...
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'strict $.foo' unknown on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo' false on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo' true on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo' unknown on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{}', 'invalid $.foo' false on error)", Boolean.FALSE );
        tester.checkBoolean( "json_exists('{}', 'invalid $.foo' true on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{}', 'invalid $.foo' unknown on error)", null );

        // not exists
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'strict $.foo1' false on error)", Boolean.FALSE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'strict $.foo1' true on error)", Boolean.TRUE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'strict $.foo1' unknown on error)", null );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo1' true on error)", Boolean.FALSE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo1' false on error)", Boolean.FALSE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo1' error on error)", Boolean.FALSE );
        tester.checkBoolean( "json_exists('{\"foo\":\"bar\"}', 'lax $.foo1' unknown on error)", Boolean.FALSE );
    }
    ...
    public void testJsonValue() {
        ...
        tester.checkString(
                ...
                "VARCHAR(2000)" );
        tester.checkScalar(
                "json_value('{\"foo\":100}', 'strict $.foo' returning integer)",
                100,
                "INTEGER" );
        tester.checkFails(
                "json_value('{\"foo\":\"100\"}', 'strict $.foo' returning boolean)",
                INVALID_CHAR_MESSAGE,
                true );
        tester.checkScalar(
                "json_value('{\"foo\":100}', 'lax $.foo1' returning integer null on empty)",
                null,
                "INTEGER" );
        tester.checkScalar(
                "json_value('{\"foo\":\"100\"}', 'strict $.foo1' returning boolean null on error)",
                null,
                "BOOLEAN" );

        // lax test
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo' null on empty)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo' error on empty)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                ...
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo1' null on empty)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_value('{\"foo\":100}', 'lax $.foo1' error on empty)",
                "(?s).*Empty result of JSON_VALUE function is not allowed.*",
                true );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo1' default 'empty' on empty)",
                "empty",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":{}}', 'lax $.foo' null on empty)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_value('{\"foo\":{}}', 'lax $.foo' error on empty)",
                "(?s).*Empty result of JSON_VALUE function is not allowed.*",
                true );
        tester.checkString(
                "json_value('{\"foo\":{}}', 'lax $.foo' default 'empty' on empty)",
                "empty",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo' null on error)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo' error on error)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'lax $.foo' default 'empty' on error)",
                "100",
                "VARCHAR(2000)" );

        // path error test
        tester.checkString(
                "json_value('{\"foo\":100}', 'invalid $.foo' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_value('{\"foo\":100}', 'invalid $.foo' error on error)",
                "(?s).*Illegal jsonpath spec.*",
                true );
        tester.checkString(
                "json_value('{\"foo\":100}', 'invalid $.foo' default 'empty' on error)",
                "empty",
                "VARCHAR(2000)" );

        // strict test
        tester.checkString(
                "json_value('{\"foo\":100}', 'strict $.foo' null on empty)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'strict $.foo' error on empty)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'strict $.foo' default 'empty' on empty)",
                "100",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":100}', 'strict $.foo1' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_value('{\"foo\":100}', 'strict $.foo1' error on error)",
                "(?s).*No results for path: \\$\\['foo1'\\].*",
                true );
        tester.checkString(
                "json_value('{\"foo\":100}', 'strict $.foo1' default 'empty' on error)",
                "empty",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_value('{\"foo\":{}}', 'strict $.foo' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_value('{\"foo\":{}}', 'strict $.foo' error on error)",
                "(?s).*Strict jsonpath mode requires scalar value, and the actual value is: '\\{\\}'.*",
                true );
        tester.checkString(
                "json_value('{\"foo\":{}}', 'strict $.foo' default 'empty' on error)",
                "empty",
                "VARCHAR(2000)" );
    }
    ...
    public void testJsonQuery() {
        // lax test
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $' null on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $' error on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $' empty array on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $' empty object on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $.foo' null on empty)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_query('{\"foo\":100}', 'lax $.foo' error on empty)",
                "(?s).*Empty result of JSON_QUERY function is not allowed.*",
                true );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $.foo' empty array on empty)",
                "[]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'lax $.foo' empty object on empty)",
                "{}",
                "VARCHAR(2000)" );

        // path error test
        tester.checkString(
                "json_query('{\"foo\":100}', 'invalid $.foo' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_query('{\"foo\":100}', 'invalid $.foo' error on error)",
                "(?s).*Illegal jsonpath spec.*",
                true );
        tester.checkString(
                "json_query('{\"foo\":100}', 'invalid $.foo' empty array on error)",
                "[]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'invalid $.foo' empty object on error)",
                "{}",
                "VARCHAR(2000)" );

        // strict test
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $' null on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $' error on empty)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $' empty array on error)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $' empty object on error)",
                "{\"foo\":100}",
                "VARCHAR(2000)" );

        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo1' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_query('{\"foo\":100}', 'strict $.foo1' error on error)",
                "(?s).*No results for path: \\$\\['foo1'\\].*",
                true );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo1' empty array on error)",
                "[]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo1' empty object on error)",
                "{}",
                "VARCHAR(2000)" );

        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' null on error)",
                null,
                "VARCHAR(2000)" );
        tester.checkFails(
                "json_query('{\"foo\":100}', 'strict $.foo' error on error)",
                "(?s).*Strict jsonpath mode requires array or object value, and the actual value is: '100'.*",
                true );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' empty array on error)",
                "[]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' empty object on error)",
                "{}",
                "VARCHAR(2000)" );

        // array wrapper test
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' without wrapper)",
                null,
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' without array wrapper)",
                null,
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' with wrapper)",
                "[100]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' with unconditional wrapper)",
                "[100]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":100}', 'strict $.foo' with conditional wrapper)",
                "[100]",
                "VARCHAR(2000)" );
        tester.checkString(
                "json_query('{\"foo\":[100]}', 'strict $.foo' without wrapper)",
    ...
    public void testJsonObject() {
        ...
        tester.checkString(
                ...
                "VARCHAR(2000) NOT NULL" );
        tester.checkString(
                "json_object('foo': null)",
                "{\"foo\":null}",
                "VARCHAR(2000) NOT NULL" );
        tester.checkString(
                "json_object('foo': null null on null)",
                "{\"foo\":null}",
                "VARCHAR(2000) NOT NULL" );
        tester.checkString(
                "json_object('foo': null absent on null)",
                "{}",
                "VARCHAR(2000) NOT NULL" );
        tester.checkString(
                "json_object('foo': 100)",
    ...
    public void testJsonObjectAgg() {
        ...
        tester.checkAggWithMultipleArgs(
                "json_objectagg(x: x2)",
                values,
                "{\"foo\":\"bar\",\"foo2\":null,\"foo3\":\"bar3\"}",
                0.0D );
        tester.checkAggWithMultipleArgs(
                "json_objectagg(x: x2 null on null)",
                values,
                "{\"foo\":\"bar\",\"foo2\":null,\"foo3\":\"bar3\"}",
                0.0D );
        tester.checkAggWithMultipleArgs(
                "json_objectagg(x: x2 absent on null)",
                values,
                "{\"foo\":\"bar\",\"foo3\":\"bar3\"}",
                0.0D );
    }
    ...
    public void testJsonArray() {
        ...
        tester.checkString( "json_array('foo', 'bar')", "[\"foo\",\"bar\"]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(null)", "[]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(null null on null)", "[null]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(null absent on null)", "[]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(100)", "[100]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(json_array('foo'))", "[\"[\\\"foo\\\"]\"]", "VARCHAR(2000) NOT NULL" );
        tester.checkString( "json_array(json_array('foo') format json)", "[[\"foo\"]]", "VARCHAR(2000) NOT NULL" );
    }
    ...
    public void testLowerFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LOWER ) );
    ...
    public void testInitcapFunc() {
        ...
        tester.setFor( StdOperatorRegistry.get( OperatorName.INITCAP ), VM_FENNEL );
    ...
    public void testPowerFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.POWER ) );
        tester.checkScalarApprox(
                "power(2,-2)",
                "DOUBLE NOT NULL",
                0.25,
    ...
    public void testSqrtFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SQRT ), SqlTester.VmName.EXPAND );
    ...
    public void testExpFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.EXP ), VM_FENNEL );
    ...
    public void testModFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MOD ) );
    ...
    public void testLnFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LN ) );
        tester.checkScalarApprox(
                "ln(2.71828)",
                "DOUBLE NOT NULL",
                1.0,
    ...
    public void testLogFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LOG10 ) );
        tester.checkScalarApprox(
                "log10(10)",
                "DOUBLE NOT NULL",
                1.0,
    ...
    public void testRandFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.RAND ) );
    ...
    public void testRandIntegerFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.RAND_INTEGER ) );
        for ( int i = 0; i < 100; i++ ) {
    ...
    public void testAbsFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ABS ) );
    ...
    public void testAcosFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ACOS ) );
    ...
    public void testAsinFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ASIN ) );
    ...
    public void testAtanFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ATAN ) );
    ...
    public void testAtan2Func() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ATAN2 ) );
    ...
    public void testCosFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COS ) );
    ...
    public void testCotFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COT ) );
    ...
    public void testDegreesFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DEGREES ) );
    ...
    public void testPiFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.PI ) );
    ...
    public void testRadiansFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.RADIANS ) );
    ...
    public void testRoundFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ROUND ) );
    ...
    public void testSignFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SIGN ) );
    ...
    public void testSinFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SIN ) );
    ...
    public void testTanFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TAN ) );
    ...
    public void testTruncateFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TRUNCATE ) );
    ...
    public void testNullifFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NULLIF ), VM_EXPAND );
        ...
        // Error message reflects the fact that Nullif is expanded before it is validated (like a C macro). Not perfect, but good enough.
        tester.checkFails( "1 + ^nullif(1, date '2005-8-4')^ + 2", "(?s)Cannot apply '=' to arguments of type '<INTEGER> = <DATE>'\\..*", false );

        tester.checkFails( "1 + ^nullif(1, 2, 3)^ + 2", "Invalid number of arguments to function 'NULLIF'\\. Was expecting 2 arguments", false );
    }
    ...
    public void testCoalesceFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COALESCE ), VM_EXPAND );
        tester.checkString( "coalesce('a','b')", "a", "CHAR(1) NOT NULL" );
        tester.checkScalarExact( "coalesce(null,null,3)", "3" );
        tester.checkFails( "1 + ^coalesce('a', 'b', 1, null)^ + 2", "Illegal mixing of types in CASE or COALESCE statement", false );
    }


    @Test
    public void testUserFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.USER ), VM_FENNEL );
    ...
    public void testCurrentUserFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_USER ), VM_FENNEL );
    ...
    public void testSessionUserFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SESSION_USER ), VM_FENNEL );
    ...
    public void testSystemUserFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SYSTEM_USER ), VM_FENNEL );
    ...
    public void testCurrentPathFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_PATH ), VM_FENNEL );
    ...
    public void testCurrentRoleFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_ROLE ), VM_FENNEL );
    ...
    public void testCurrentCatalogFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_CATALOG ), VM_FENNEL );
    ...
    public void testLocalTimeFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LOCALTIME ) );
        ...
        final Pair<String, Closeable> pair = currentTimeString( LOCAL_TZ );
        tester.checkScalar(
    ...
    public void testLocalTimestampFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LOCALTIMESTAMP ) );
        tester.checkScalar(
                "LOCALTIMESTAMP",
                TIMESTAMP_PATTERN,
                "TIMESTAMP(0) NOT NULL" );
        ...
        tester.checkFails(
                ...
                false );
        tester.checkFails(
                "^LOCALTIMESTAMP(9223372036854775807)^",
                LITERAL_OUT_OF_RANGE_MESSAGE,
                false );
        tester.checkScalar(
                "LOCALTIMESTAMP(1)",
        ...
        // Check that timestamp is being generated in the right timezone by generating a specific timestamp.
        final Pair<String, Closeable> pair = currentTimeString(
                LOCAL_TZ );
        tester.checkScalar(
    ...
    public void testCurrentTimeFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_TIME ) );
        tester.checkScalar(
        ...
        final Pair<String, Closeable> pair = currentTimeString( CURRENT_TZ );
        tester.checkScalar(
    ...
    public void testCurrentTimestampFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_TIMESTAMP ) );
        tester.checkScalar(
                "CURRENT_TIMESTAMP",
                TIMESTAMP_PATTERN,
                "TIMESTAMP(0) NOT NULL" );
     ...
     * @return Time string
     */
    protected static Pair<String, Closeable> currentTimeString( TimeZone tz ) {
        ...
        if ( PolyphenyDbAssert.ENABLE_SLOW ) {
            ...
            closeable = Hook.CURRENT_TIME.addThread( (Consumer<Holder<Long>>) o -> o.set( timeInMillis ) );
        }
    ...
    public void testCurrentDateFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CURRENT_DATE ), VM_FENNEL );
    ...
    public void testSubstringFunction() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SUBSTRING ) );
        tester.checkString(
        ...
        tester.checkString(
                ...
                "VARCHAR(3) NOT NULL" );
        tester.checkString(
                "substring('abc' from 0)",
                "abc",
                "VARCHAR(3) NOT NULL" );
        tester.checkString(
                "substring('abc' from 8)",
                "",
                "VARCHAR(3) NOT NULL" );
        tester.checkString(
                "substring('abc' from -2)",
        ...
        tester.checkFails(
                ...
                true );
        tester.checkString(
                "substring(x'aabbcc' from 2)",
                "bbcc",
                "VARBINARY(3) NOT NULL" );
        tester.checkString(
                "substring(x'aabbcc' from 0)",
                "aabbcc",
                "VARBINARY(3) NOT NULL" );
        tester.checkString(
                "substring(x'aabbcc' from 8)",
                "",
                "VARBINARY(3) NOT NULL" );
        tester.checkString(
                "substring(x'aabbcc' from -2)",
    ...
    public void testTrimFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TRIM ) );
        ...
        // SQL:2003 6.29.9: trim string must have length=1. Failure occurs at runtime.
        //
        // TODO: Change message to "Invalid argument\(s\) for 'TRIM' function". The message should come from a resource file, and should still have the SQL error code 22027.
        tester.checkFails(
                "trim('xy' from 'abcde')",
    ...
    */
    ...
    public void testElementFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ELEMENT ), VM_FENNEL, VM_JAVA );
    ...
    public void testCardinalityFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CARDINALITY ), VM_FENNEL, VM_JAVA );
    ...
    public void testMemberOfOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MEMBER_OF ), VM_FENNEL, VM_JAVA );
    ...
    public void testMultisetUnionOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MULTISET_UNION_DISTINCT ), VM_FENNEL, VM_JAVA );
        tester.checkBoolean( "multiset[1,2] submultiset of (multiset[2] multiset union multiset[1])", Boolean.TRUE );
        tester.checkScalar(
                "cardinality(multiset[1, 2, 3, 4, 2] multiset union distinct multiset[1, 4, 5, 7, 8])",
                "7",
                "INTEGER NOT NULL" );
        tester.checkScalar(
                "cardinality(multiset[1, 2, 3, 4, 2] multiset union distinct multiset[1, 4, 5, 7, 8])",
                ...
                "INTEGER NOT NULL" );
        tester.checkBoolean(
                "(multiset[1, 2, 3, 4, 2] multiset union distinct multiset[1, 4, 5, 7, 8]) submultiset of multiset[1, 2, 3, 4, 5, 7, 8]",
                Boolean.TRUE );
        tester.checkBoolean(
                "(multiset[1, 2, 3, 4, 2] multiset union distinct multiset[1, 4, 5, 7, 8]) submultiset of multiset[1, 2, 3, 4, 5, 7, 8]",
                Boolean.TRUE );
        tester.checkScalar(
                "cardinality(multiset['a', 'b', 'c'] multiset union distinct multiset['c', 'd', 'e'])",
                "5",
                "INTEGER NOT NULL" );
        tester.checkScalar(
                "cardinality(multiset['a', 'b', 'c'] multiset union distinct multiset['c', 'd', 'e'])",
                ...
                "INTEGER NOT NULL" );
        tester.checkBoolean(
                "(multiset['a', 'b', 'c'] multiset union distinct multiset['c', 'd', 'e']) submultiset of multiset['a', 'b', 'c', 'd', 'e']",
                Boolean.TRUE );
        tester.checkBoolean(
                "(multiset['a', 'b', 'c'] multiset union distinct multiset['c', 'd', 'e']) submultiset of multiset['a', 'b', 'c', 'd', 'e']",
                Boolean.TRUE );
    ...
    public void testMultisetUnionAllOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MULTISET_UNION ), VM_FENNEL, VM_JAVA );
        tester.checkScalar(
                "cardinality(multiset[1, 2, 3, 4, 2] multiset union all multiset[1, 4, 5, 7, 8])",
                "10",
                "INTEGER NOT NULL" );
        tester.checkBoolean(
                "(multiset[1, 2, 3, 4, 2] multiset union all multiset[1, 4, 5, 7, 8]) submultiset of multiset[1, 2, 3, 4, 5, 7, 8]",
                Boolean.FALSE );
        tester.checkBoolean(
                "(multiset[1, 2, 3, 4, 2] multiset union all multiset[1, 4, 5, 7, 8]) submultiset of multiset[1, 1, 2, 2, 3, 4, 4, 5, 7, 8]",
                Boolean.TRUE );
        tester.checkScalar(
                "cardinality(multiset['a', 'b', 'c'] multiset union all multiset['c', 'd', 'e'])",
                "6",
                "INTEGER NOT NULL" );
        tester.checkBoolean(
                "(multiset['a', 'b', 'c'] multiset union all multiset['c', 'd', 'e']) submultiset of multiset['a', 'b', 'c', 'd', 'e']",
                Boolean.FALSE );
        tester.checkBoolean(
                "(multiset['a', 'b', 'c'] multiset union distinct multiset['c', 'd', 'e']) submultiset of multiset['a', 'b', 'c', 'd', 'e', 'c']",
                Boolean.TRUE );
        tester.checkScalar(
    ...
    public void testSubMultisetOfOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SUBMULTISET_OF ), VM_FENNEL, VM_JAVA );
        ...
        tester.checkBoolean( "multiset['a', 'b'] submultiset of multiset['c', 'd', 's', 'a']", Boolean.FALSE );
        tester.checkBoolean( "multiset['a', 'd'] submultiset of multiset['c', 's', 'a', 'w', 'd']", Boolean.TRUE );
    ...
    public void testNotSubMultisetOfOperator() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.NOT_SUBMULTISET_OF ), VM_FENNEL, VM_JAVA );
        ...
        tester.checkBoolean( "multiset['a', 'b'] not submultiset of multiset['c', 'd', 's', 'a']", Boolean.TRUE );
        tester.checkBoolean( "multiset['a', 'd'] not submultiset of multiset['c', 's', 'a', 'w', 'd']", Boolean.FALSE );
    ...
    public void testCollectFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COLLECT ), VM_FENNEL, VM_JAVA );
    ...
    public void testFusionFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.FUSION ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testYear() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.YEAR ), VM_FENNEL, VM_JAVA );
    ...
    public void testQuarter() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.QUARTER ), VM_FENNEL, VM_JAVA );
    ...
    public void testMonth() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MONTH ), VM_FENNEL, VM_JAVA );
    ...
    public void testWeek() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.WEEK ), VM_FENNEL, VM_JAVA );
        if ( Bug.CALCITE_2539_FIXED ) {
    ...
    public void testDayOfYear() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DAYOFYEAR ), VM_FENNEL, VM_JAVA );
        if ( Bug.CALCITE_2539_FIXED ) {
            // TODO: Not implemented in operator test execution code
            tester.checkFails(
    ...
    public void testDayOfMonth() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DAYOFMONTH ), VM_FENNEL, VM_JAVA );
        tester.checkScalar(
    ...
    public void testDayOfWeek() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DAYOFWEEK ), VM_FENNEL, VM_JAVA );
        if ( Bug.CALCITE_2539_FIXED ) {
            // TODO: Not implemented in operator test execution code
            tester.checkFails(
    ...
    public void testHour() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.HOUR ), VM_FENNEL, VM_JAVA );
    ...
    public void testMinute() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MINUTE ), VM_FENNEL, VM_JAVA );
    ...
    public void testSecond() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SECOND ), VM_FENNEL, VM_JAVA );
    ...
    public void testExtractIntervalYearMonth() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.EXTRACT ), VM_FENNEL, VM_JAVA );

        if ( TODO ) {
            // Not supported, fails in type validation because the extract unit is not YearMonth interval type.

            tester.checkScalar(
                    "extract(epoch from interval '4-2' year to month)",
                    // number of seconds elapsed since timestamp '1970-01-01 00:00:00' + input interval
                    "131328000",
                    "BIGINT NOT NULL" );
    ...
    public void testExtractIntervalDayTime() {
        ...
        // Postgres doesn't support DOW, ISODOW, DOY and WEEK on INTERVAL DAY TIME type. SQL standard doesn't have extract units for DOW, ISODOW, DOY and WEEK.
        if ( Bug.CALCITE_2539_FIXED ) {
            tester.checkFails(
                    "extract(doy from interval '2 3:4:5.678' day to second)",
                    INVALID_EXTRACT_UNIT_CONVERTLET_ERROR,
                    true );
            tester.checkFails(
                    "extract(dow from interval '2 3:4:5.678' day to second)",
                    INVALID_EXTRACT_UNIT_CONVERTLET_ERROR,
                    true );
            tester.checkFails(
                    "extract(week from interval '2 3:4:5.678' day to second)",
                    INVALID_EXTRACT_UNIT_CONVERTLET_ERROR,
                    true );
            tester.checkFails(
                    "extract(isodow from interval '2 3:4:5.678' day to second)",
                    INVALID_EXTRACT_UNIT_CONVERTLET_ERROR,
                    true );
        }
    ...
    public void testExtractDate() {
        ...
        tester.checkScalar(
                ...
                "BIGINT NOT NULL" );
        tester.checkScalar(
                "extract(century from date '1852-06-07')",
                "19",
                "BIGINT NOT NULL" );
        tester.checkScalar(
                "extract(century from date '0001-02-01')",
                "1",
                "BIGINT NOT NULL" );

        tester.checkScalar(
                "extract(millennium from date '2000-2-23')",
                "2",
                "BIGINT NOT NULL" );
        tester.checkScalar(
                "extract(millennium from date '1969-2-23')",
                "2",
                "BIGINT NOT NULL" );
        tester.checkScalar(
    ...
    public void testExtractTimestamp() {
        ...
        tester.checkScalar(
                ...
                "1203770096", // number of seconds elapsed since timestamp '1970-01-01 00:00:00' for given date
                "BIGINT NOT NULL" );
    ...
    public void testExtractFunc() {
        tester.setFor(
                StdOperatorRegistry.get( OperatorName.EXTRACT ),
                VM_FENNEL,
                VM_JAVA );
    ...
    public void testExtractFuncFromDateTime() {
        ...
        tester.checkNull( "extract(millisecond from cast(null as time))" );
        tester.checkNull( "extract(microsecond from cast(null as time))" );
    }


    @Test
    public void testArrayValueConstructor() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ARRAY_VALUE_CONSTRUCTOR ) );
        tester.checkScalar(
                "Array['foo', 'bar']",
                "[foo, bar]",
    ...
    public void testItemOp() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ITEM ) );
        ...
        // Array of INTEGER NOT NULL is interesting because we might be tempted to represent the result as Java "int".
    ...
    public void testMapValueConstructor() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MAP_VALUE_CONSTRUCTOR ), VM_JAVA );
    ...
    public void testCeilFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CEIL ), VM_FENNEL );
    ...
    public void testFloorFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.FLOOR ), VM_FENNEL );
    ...
    public void testFloorFuncDateTime() {
        ...
        tester.checkFails(
                "^floor(time '12:34:56')^",
                "(?s)Cannot apply 'FLOOR' to arguments .*",
                false );
        tester.checkFails(
                "^floor(123.45 to minute)^",
                "(?s)Cannot apply 'FLOOR' to arguments .*",
                false );
        tester.checkFails(
                "^floor('abcde' to minute)^",
                "(?s)Cannot apply 'FLOOR' to arguments .*",
                false );
        tester.checkScalar(
                "floor(time '12:34:56' to minute)",
                "12:34:00",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "floor(timestamp '2015-02-19 12:34:56.78' to second)",
                "2015-02-19 12:34:56",
                "TIMESTAMP(2) NOT NULL" );
        tester.checkScalar(
                "floor(timestamp '2015-02-19 12:34:56' to minute)",
                "2015-02-19 12:34:00",
                "TIMESTAMP(0) NOT NULL" );
        tester.checkScalar(
                "floor(timestamp '2015-02-19 12:34:56' to year)",
                "2015-01-01 00:00:00",
                "TIMESTAMP(0) NOT NULL" );
        tester.checkScalar(
                "floor(timestamp '2015-02-19 12:34:56' to month)",
                "2015-02-01 00:00:00",
                "TIMESTAMP(0) NOT NULL" );
    ...
    public void testCeilFuncDateTime() {
        ...
        tester.checkFails(
                "^ceil(time '12:34:56')^",
                "(?s)Cannot apply 'CEIL' to arguments .*",
                false );
        tester.checkFails(
                "^ceil(123.45 to minute)^",
                "(?s)Cannot apply 'CEIL' to arguments .*",
                false );
        tester.checkFails(
                "^ceil('abcde' to minute)^",
                "(?s)Cannot apply 'CEIL' to arguments .*",
                false );
        tester.checkScalar(
        ...
        tester.checkScalar(
                ...
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "ceil(timestamp '2015-02-19 12:34:56.78' to second)",
                "2015-02-19 12:34:57",
                "TIMESTAMP(2) NOT NULL" );
        tester.checkScalar(
                "ceil(timestamp '2015-02-19 12:34:56.00' to second)",
                "2015-02-19 12:34:56",
                "TIMESTAMP(2) NOT NULL" );
        tester.checkScalar(
                "ceil(timestamp '2015-02-19 12:34:56' to minute)",
                "2015-02-19 12:35:00",
                "TIMESTAMP(0) NOT NULL" );
        tester.checkScalar(
                "ceil(timestamp '2015-02-19 12:34:56' to year)",
                "2016-01-01 00:00:00",
                "TIMESTAMP(0) NOT NULL" );
        tester.checkScalar(
                "ceil(timestamp '2015-02-19 12:34:56' to month)",
                "2015-03-01 00:00:00",
                "TIMESTAMP(0) NOT NULL" );
        ...
        // ceiling alias
        tester.checkScalar(
                "ceiling(timestamp '2015-02-19 12:34:56' to month)",
                "2015-03-01 00:00:00",
                "TIMESTAMP(0) NOT NULL" );
    ...
    public void testTimestampAdd() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TIMESTAMP_ADD ) );
        tester.checkScalar(
        ...
        tester.checkScalar(
                ...
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(DAY, 15, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(WEEK, 3, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(MONTH, 6, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(QUARTER, 1, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(YEAR, 10, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        // TIMESTAMPADD with time; returns a time value .The interval is negative.
        tester.checkScalar(
                "timestampadd(SECOND, -1, time '00:00:00')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(MINUTE, -1, time '00:00:00')",
                ...
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(HOUR, -1, time '00:00:00')",
                "23:00:00",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(DAY, -1, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(WEEK, -1, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(MONTH, -1, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(QUARTER, -1, time '23:59:59')",
                ...
                "TIME(0) NOT NULL" );
        tester.checkScalar(
                "timestampadd(YEAR, -1, time '23:59:59')",
                "23:59:59",
                "TIME(0) NOT NULL" );
    }
    ...
    public void testTimestampDiff() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.TIMESTAMP_DIFF ) );
        tester.checkScalar(
        ...
        tester.checkScalar(
                ...
                "BIGINT NOT NULL" );
        tester.checkScalar(
                "timestampdiff(NANOSECOND, timestamp '2016-02-24 12:42:25', timestamp '2016-02-24 12:42:20')",
                "-5000000000",
                "BIGINT NOT NULL" );
        tester.checkScalar(
    ...
    public void testDenseRankFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.DENSE_RANK ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testPercentRankFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.PERCENT_RANK ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testRankFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.RANK ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testCumeDistFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.CUME_DIST ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testRowNumberFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ROW_NUMBER ), VM_FENNEL, VM_JAVA );
    }


    @Test
    public void testCountFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COUNT ), VM_EXPAND );
        ...
        tester.checkFails( "^COUNT()^", "Invalid number of arguments to function 'COUNT'. Was expecting 1 arguments", false );
        tester.checkType( "count(1, 2)", "BIGINT NOT NULL" );
    ...
    public void testApproxCountDistinctFunc() {
        ...
        tester.checkFails(
                ...
                "Invalid number of arguments to function 'APPROX_COUNT_DISTINCT'. Was expecting 1 arguments",
                false );
        tester.checkType( "approx_count_distinct(1, 2)", "BIGINT NOT NULL" );
    ...
    public void testSumFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.SUM ), VM_EXPAND );
        tester.checkFails(
                "sum(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testAvgFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.AVG ), VM_EXPAND );
        tester.checkFails(
                "avg(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testCovarPopFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COVAR_POP ), VM_EXPAND );
    ...
    public void testCovarSampFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.COVAR_SAMP ), VM_EXPAND );
        tester.checkFails(
                "covar_samp(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testRegrSxxFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.REGR_SXX ), VM_EXPAND );
        tester.checkFails(
                "regr_sxx(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testRegrSyyFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.REGR_SYY ), VM_EXPAND );
        tester.checkFails(
                "regr_syy(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testStddevPopFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.STDDEV_POP ), VM_EXPAND );
        tester.checkFails(
                "stddev_pop(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testStddevSampFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.STDDEV_SAMP ), VM_EXPAND );
        tester.checkFails(
                "stddev_samp(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testStddevFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.STDDEV ), VM_EXPAND );
        tester.checkFails(
                "stddev(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testVarPopFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.VAR_POP ), VM_EXPAND );
        tester.checkFails(
                "var_pop(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testVarSampFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.VAR_SAMP ), VM_EXPAND );
        tester.checkFails(
                "var_samp(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testVarFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.VARIANCE ), VM_EXPAND );
        tester.checkFails(
                "variance(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testMinFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MIN ), VM_EXPAND );
        tester.checkFails(
                "min(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testMaxFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.MAX ), VM_EXPAND );
        tester.checkFails(
                "max(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testLastValueFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.LAST_VALUE ), VM_EXPAND );
    ...
    public void testFirstValueFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.FIRST_VALUE ), VM_EXPAND );
        final String[] values = { "0", "CAST(null AS INTEGER)", "3", "3" };
        if ( !enable ) {
            return;
        }
    ...
    public void testAnyValueFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.ANY_VALUE ), VM_EXPAND );
        tester.checkFails(
                "any_value(^*^)",
                "Unknown identifier '\\*'",
                false );
    ...
    public void testBitAndFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.BIT_AND ), VM_FENNEL, VM_JAVA );
    ...
    public void testBitOrFunc() {
        tester.setFor( StdOperatorRegistry.get( OperatorName.BIT_OR ), VM_FENNEL, VM_JAVA );
                ...
                for ( final List<ValueType> args : Linq4j.product( argValues ) ) {
                    ...
                    try {
                        if ( nullCount > 0 && policy == Strong.Policy.ANY ) {
                            ...
                            tester.check( query, SqlTests.ANY_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER, result -> {
                            } );
                        }
                    } catch ( Error e ) {
    ...
    static class ValueType {
        ...
        private SqlNode literal( RelDataType type, Object value ) {
            if ( value == null ) {
                ...
                return (SqlNode) StdOperatorRegistry.get( OperatorName.CAST ).createCall(
