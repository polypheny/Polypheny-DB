...
import org.polypheny.db.adaptimizer.rndqueries.*;
...
public class ReAdaptiveOptimizerImpl extends AdaptiveOptimizerImpl {
    ...
    public String createSession( RelQueryTemplate relQueryTemplate, int nrOfQueries ) {
        final String sessionId = UUID.randomUUID().toString();
        OptSession optSession = new RelQuerySession( new QuerySupplier( RelQueryGenerator.from(relQueryTemplate) ) );
        final SessionData sessionData = new SessionData( sessionId, relQueryTemplate, new Thread( optSession ), optSession, nrOfQueries );
    ...
    public void joinStartedSession( String sid ) {
        SessionData session = getSessions().get( sid );
        session.start();
        try {
            session.getSessionThread().join();
