...
public class Filter extends Unary {
    ...
    public static Filter filter( Column filterField, OperatorName filterOperator, Object filterVal ) {
        Filter filter = new Filter();

        filter.setOperatorType( OperatorType.FILTER );

        filter.setFilterField( filterField );
        filter.setFilterOperator( filterOperator );
        filter.setTarget( filterField.getResult().getNode() );
        filter.setFilter( filterVal );

        filter.setResult( Result.from( filter ) );
