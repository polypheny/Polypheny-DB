...
final class ElasticsearchTransport {
    ...
    ElasticsearchTransport( final RestClient restClient, final ObjectMapper mapper, final String indexName, final String typeName, final int fetchSize ) {
    ...
    /**
     * Detects current Elastic Search version by connecting to a existing instance. It is a {@code GET} request to {@code /}. Returned JSON has server information (including version).
     *
    ...
    private ElasticsearchVersion version() {
        ...
        final Function<ObjectNode, ElasticsearchVersion> fn = node -> ElasticsearchVersion.fromString( node.get( "version" ).get( "number" ).asText() );
    ...
    private static class JsonParserFn<T> implements Function<Response, T> {
        ...
        public T apply( final Response response ) {
            try ( InputStream is = response.getEntity().getContent() ) {
                ...
                final String message = String.format( Locale.ROOT, "Couldn't parse HTTP response %s into %s", response, klass );
                throw new UncheckedIOException( message, e );
            }
        }
    }
    ...
    private static class HttpFunction implements Function<HttpRequest, Response> {
        ...
        private Response applyInternal( final HttpRequest request )
            ...
            final HttpEntity entity = request instanceof HttpEntityEnclosingRequest ? ((HttpEntityEnclosingRequest) request).getEntity() : null;

            final Response response = restClient.performRequest( request.getRequestLine().getMethod(), request.getRequestLine().getUri(), Collections.emptyMap(), entity );

            final String payload = entity != null && entity.isRepeatable() ? EntityUtils.toString( entity ) : "<empty>";

            if ( response.getStatusLine().getStatusCode() != HttpStatus.SC_OK ) {
                final String error = EntityUtils.toString( response.getEntity() );

                final String message = String.format( Locale.ROOT, "Error while querying Elastic (on %s/%s) status: %s\nPayload:\n%s\nError:\n%s\n", response.getHost(), response.getRequestLine(), response.getStatusLine(), payload, error );
