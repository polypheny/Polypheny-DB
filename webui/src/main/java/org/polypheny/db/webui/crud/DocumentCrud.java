...
public class DocumentCrud {
    ...
    public List<Result> anyQuery( Session session, QueryRequest request, Crud crud ) {
        ...
        if ( parsed.getFamily() == Family.DML && mqlProcessor.needsDdlGeneration( parsed, request.database ) ) {
            mqlProcessor.autoGenerateDDL( crud.getTransaction( request.analyze ).createStatement(), parsed, request.database );
        }

        long executionTime = System.nanoTime();

        if ( parsed.getFamily() == Family.DDL ) {
            mqlProcessor.prepareDdl( statement, parsed, mql, request.database );
