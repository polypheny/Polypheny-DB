...
public class MysqlSourceTest extends AbstractSourceTest {
    ...
    public static void start() throws SQLException {
        ...
        // Add adapter
        int mariadbPort = server.getConfig().getPort();
        try ( JdbcConnection polyphenyDbConnection = new JdbcConnection( true ) ) {
            Connection connection = polyphenyDbConnection.getConnection();
            try ( Statement statement = connection.createStatement() ) {
                statement.executeUpdate( "ALTER ADAPTERS ADD mariadbunit USING 'org.polypheny.db.adapter.jdbc.sources.MysqlSource' WITH '{database:\"test\",host:\"localhost\",maxConnections:\"25\",password:\"\",username:\"root\",port:\"" + mariadbPort + "\",transactionIsolation:\"SERIALIZABLE\",tables:\"auction,bid,category,picture,user\"}'" );
            }
        }
    }
