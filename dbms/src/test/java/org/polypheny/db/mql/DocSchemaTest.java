...
/**
 * Covers CREATE COLLECTION schema parsing for root object rules, required handling,
 * additionalProperties, scalar unions, composition nodes, nested objects, and array declarations.
 */
@Tag("adapter")
    ...
    public void createCollection_withUnknownPropertyType_shouldFail() {
        ...
        assertThrows( Exception.class, () -> execute(
                "db.createCollection(\"" + USER + "\", {"
                        + "  docSchema: {"
                        + "    type: \"object\","
                        + "    properties: { name: { type: \"wrongType\" } },"
                        + "    additionalProperties: true"
                        + "  },"
                        + "  validationAction: \"strict\""
                        + "})" ) );
