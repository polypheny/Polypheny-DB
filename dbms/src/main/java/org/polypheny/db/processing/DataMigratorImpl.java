...
public class DataMigratorImpl implements DataMigrator {
    ...
    public void executeQuery( List<AllocationColumn> selectedColumns, AlgRoot sourceAlg, Statement sourceStatement, Statement targetStatement, AlgRoot targetAlg, boolean isMaterializedView, boolean doesSubstituteOrderBy ) {
        try {
            ...
            int i = 0;
            do {
                List<List<PolyValue>> rows = result.open( sourceStatement, batchSize, false ).getRows();
                if ( rows.isEmpty() ) {
                    continue;
                }
     ...
     */
    @Override
    public void copyAllocationData( Transaction transaction, CatalogAdapter store, List<AllocationTable> sourceTables, PartitionProperty targetProperty, List<AllocationTable> targetTables, LogicalTable table ) {
        ...
        try {
            ...
            do {
                List<List<PolyValue>> rows = result.open( source.sourceStatement, batchSize, false ).getRows();//MetaImpl.collect( result.getCursorFactory(), LimitIterator.of( sourceIterator, batchSize ), new ArrayList<>() ).stream().map( r -> r.stream().map( e -> (PolyValue) e ).collect( Collectors.toList() ) ).collect( Collectors.toList() );
