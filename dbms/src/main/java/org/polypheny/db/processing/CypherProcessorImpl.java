...
public class CypherProcessorImpl extends AutomaticDdlProcessor {
    ...
    public void autoGenerateDDL( Statement statement, Node node, QueryParameters parameters ) {
        ...
        try {
            ...
            List<String> nodeLabels = cypherParameters.getNodeLabels();
            List<String> relLabels = cypherParameters.getRelationshipLabels();

            ddlManager.createGraph( 0, true, false );
            ddlManager.addGraphLabels( 0, nodeLabels, relLabels );
    ...
    public boolean needsDdlGeneration( Node query, QueryParameters parameters ) {
        ...
        ((CypherStatement) query).accept( labelExtractor );

        assert parameters instanceof CypherQueryParameters;
        CypherQueryParameters cypherParameters = (CypherQueryParameters) parameters;
        cypherParameters.getNodeLabels().clear();
        cypherParameters.getNodeLabels().addAll( labelExtractor.nodeLabels );
        cypherParameters.getRelationshipLabels().clear();
        cypherParameters.getRelationshipLabels().addAll( labelExtractor.relationshipLabels );

        if ( labelExtractor.nodeLabels.isEmpty() && labelExtractor.relationshipLabels.isEmpty() ) {
            return false;
        }
        return true;
    }
