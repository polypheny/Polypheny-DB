...
public class DdlManagerImpl extends DdlManager {
    ...
    @Override
    public void createColumn( String columnName, LogicalTable table, String beforeColumnName, String afterColumnName, ColumnTypeInformation type, boolean nullable, String defaultValue, Statement statement ) {
    ...
    public void createTablePartition( PartitionInformation partitionInfo, List<DataStore<?>> stores, Statement statement ) throws TransactionException {
        ...
        for ( AllocationPlacement placement : placements ) {
            ...
            if ( !sourceAllocs.get( 0 ).unwrap( AllocationTable.class ).getColumnIds().contains( partitionColumn.id ) ) {
                ...
                for ( AllocationPlacement allocationPlacement : placements ) {
                    ...
                    if ( allocation.unwrap( AllocationTable.class ).getColumnIds().contains( partitionColumn.id ) ) {
            ...
            dataMigrator.copyAllocationData(
                    ...
                    sourceAllocs.stream().map( s -> s.unwrap( AllocationTable.class ) ).collect( Collectors.toList() ),
                    result.right,
