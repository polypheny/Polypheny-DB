...
public class SqlAlterTableDropIndex extends SqlAlterTable {
    ...
    public void execute( Context context, Statement statement, QueryParameters parameters ) {
        ...
        if ( catalogEntity.entityType != EntityType.ENTITY && catalogEntity.entityType != EntityType.MATERIALIZED_VIEW ) {
            throw new RuntimeException( "Not possible to use ALTER TABLE DROP INDEX because " + catalogEntity.name + " is not a table or materialized view." );
        }

        try {
            DdlManager.getInstance().dropIndex( catalogEntity, indexName.getSimple(), statement );
