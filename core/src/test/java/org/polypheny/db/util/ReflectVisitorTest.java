...
/**
 * ReflectVisitorTest tests {@link ReflectUtil#invokeVisitor} and {@link ReflectiveVisitor} and provides a contrived example of how to use them.
 */
public class ReflectVisitorTest {
    ...
    public void testCarefulNegater() {
        ...
        // verify that negater is capable of handling integers, and that result comes back with same type
        result = negater.negate( 5 );
        assertEquals( -5, result.intValue() );
        assertTrue( result instanceof Integer );

        // verify that negater is capable of handling longs; even though it doesn't provide an explicit implementation, it should inherit the one from CarelessNumberNegater
    ...
    public void testCluelessNegater() {
        ...
        // verify that negater is capable of handling shorts, and that result comes back with same type
    ...
    /**
     * NumberNegater defines the abstract base for a computation object capable of negating an arbitrary number. Subclasses implement the computation by publishing methods
     * with the signature "void visit(X x)" where X is a subclass of Number.
     */
    public abstract class NumberNegater implements ReflectiveVisitor {
         ...
         * @return the negated result; not guaranteed to be the same concrete type as n; null is returned if n's type wasn't handled
         */
        public Number negate( Number n ) {
            // we specify Number.class as the hierarchy root so that extraneous visit methods are ignored
            result = null;
        ...
        /**
         * Negates the given number without using a dispatcher object to cache applicable methods. The results should be the same as {@link #negate(Number)}.
         *
         * @param n the number to be negated
         * @return the negated result; not guaranteed to be the same concrete type as n; null is returned if n's type wasn't handled
         */
    ...
    /**
     * CarefulNumberNegater implements NumberNegater in a careful fashion by providing overloads for each known subclass of Number and returning the same subclass for the result. Extends CarelessNumberNegater so that it
     ...
     * This is just here for testing the hierarchyRoot parameter of invokeVisitor.
     */
