...
public class EnumerablesTest {
    ...
    public void testSemiJoin() {
        assertThat(
                EnumerableDefaults.semiJoin( EMPS, DEPTS, e -> e.deptno, d -> d.deptno, Functions.identityComparer() ).toList().toString(),
    ...
    public void testMergeJoin2() {
        ...
        assertThat(
                intersect( Lists.newArrayList( 1, 3, 4 ), Lists.newArrayList( 1, 4 ) ).toList().toString(),
                equalTo( "[1, 4]" ) );
        // Matching key at start and end of right, not of left
        assertThat(
                intersect( Lists.newArrayList( 0, 1, 3, 4, 5 ), Lists.newArrayList( 1, 4 ) ).toList().toString(),
                equalTo( "[1, 4]" ) );
        // Matching key at start and end of left, not right
        assertThat(
                intersect( Lists.newArrayList( 1, 3, 4 ), Lists.newArrayList( 0, 1, 4, 5 ) ).toList().toString(),
                equalTo( "[1, 4]" ) );
        // Matching key not at start or end of left or right
        assertThat(
                intersect( Lists.newArrayList( 0, 2, 3, 4, 5 ), Lists.newArrayList( 1, 3, 4, 6 ) ).toList().toString(),
                equalTo( "[3, 4]" ) );
    }
    ...
    public void testMergeJoin3() {
        // No overlap
        assertThat(
                intersect( Lists.newArrayList( 0, 2, 4 ), Lists.newArrayList( 1, 3, 5 ) ).toList().toString(),
                equalTo( "[]" ) );
        // Left empty
        assertThat(
                intersect( new ArrayList<>(), newArrayList( 1, 3, 4, 6 ) ).toList().toString(),
                equalTo( "[]" ) );
        // Right empty
        assertThat(
                intersect( newArrayList( 3, 7 ), new ArrayList<>() ).toList().toString(),
                equalTo( "[]" ) );
        // Both empty
        assertThat(
                intersect( new ArrayList<Integer>(), new ArrayList<>() ).toList().toString(),
                equalTo( "[]" ) );
    }
