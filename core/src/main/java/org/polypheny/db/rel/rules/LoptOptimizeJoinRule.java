 ...
 *
 * It is triggered by the pattern {@link org.polypheny.db.rel.logical.LogicalProject} ({@link MultiJoin}).
 */
public class LoptOptimizeJoinRule extends RelOptRule {
        ...
        while ( !removalCandidates.isEmpty() ) {
            ...
            for ( int factIdx : removalCandidates ) {
                ...
                for ( RexNode filter : ojFilters ) {
                    ...
                    if ( (filterCall.getOperator() != StdOperatorRegistry.get( "EQUALS" ))
                ...
                if ( RelMdUtil.areColumnsDefinitelyUniqueWhenNullsFiltered( mq, multiJoin.getJoinFactor( factIdx ), joinKeys ) ) {
     ...
     */
    private boolean isSelfJoinFilterUnique( RelMetadataQuery mq, LoptMultiJoin multiJoin, int leftFactor, int rightFactor, List<RexNode> joinFilterList ) {
     ...
     */
    private void findBestOrderings( RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin, LoptSemiJoinOptimizer semiJoinOpt, RelOptRuleCall call ) {
     ...
     */
    private int getBestNextFactor( RelMetadataQuery mq, LoptMultiJoin multiJoin, BitSet factorsToAdd, BitSet factorsAdded, LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree, List<RexNode> filtersToAdd ) {
    ...
    private int rowWidthCost( RelNode tree ) {
        // The width cost is the width of the tree itself plus the widths of its children.  Hence, skinnier rows are better when they're lower in the tree since the width of a RelNode contributes to
     ...
     */
    private boolean swapInputs( RelMetadataQuery mq, LoptMultiJoin multiJoin, LoptJoinTree left, LoptJoinTree right, boolean selfJoin ) {
        boolean swap = false;
