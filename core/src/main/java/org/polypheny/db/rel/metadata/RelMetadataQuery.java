...
/**
 * RelMetadataQuery provides a strongly-typed facade on top of {@link RelMetadataProvider} for the set of relational expression metadata queries defined as standard within Polypheny-DB.
 ...
 * </ol>
 *
 * Because relational expression metadata is extensible, extension projects can define similar facades in order to specify access to custom metadata. Please do not add queries here (nor on {@link RelNode}) which lack meaning
 ...
 * process is the same: write a reflective provider and chain it on to an instance of {@link DefaultRelMetadataProvider}, pre-pending it to the default providers. Then supply that instance to the planner via the appropriate
 * plugin mechanism.
 */
public class RelMetadataQuery {
     ...
     * @see #getColumnOrigins(RelNode, int)
     */
    ...
    /**
     * Determines the origin of a {@link RelNode}, provided it maps to a single table, optionally with filtering and projection.
     *
    ...
    public RelOptTable getTableOrigin( RelNode rel ) {
        // Determine the simple origin of the first column in the/ RelNode. If it's simple, then that means that the underlying table is also simple, even if the column itself is derived.
    ...
    /**
     * As {@link #getAverageColumnSizes(RelNode)} but never returns a null list, only ever a list of nulls.
     */
