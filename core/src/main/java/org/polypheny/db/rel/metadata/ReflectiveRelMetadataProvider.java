...
/**
 * Implementation of the {@link RelMetadataProvider} interface that dispatches metadata methods to methods on a given object via reflection.
 *
 * The methods on the target object must be public and non-static, and have the same signature as the implemented metadata method except for an additional first parameter of type {@link RelNode} or a sub-class.
 * That parameter gives this provider an indication of that relational expressions it can handle.
 *
 * For an example, see {@link RelMdColumnOrigins#SOURCE}.
 */
public class ReflectiveRelMetadataProvider implements RelMetadataProvider, ReflectiveVisitor {
     ...
     */
    protected ReflectiveRelMetadataProvider( ConcurrentMap<Class<RelNode>, UnboundMetadata> map, Class<? extends Metadata> metadataClass0, Multimap<Method, MetadataHandler> handlerMap ) {
    ...
    /**
     * Returns an implementation of {@link RelMetadataProvider} that scans for methods with a preceding argument.
     *
    ...
    static class Space {
        ...
        Space( Multimap<Method, MetadataHandler> providerMap ) {
            ...
            // Find the distinct set of RelNode classes handled by this provider, ordered base-class first.
