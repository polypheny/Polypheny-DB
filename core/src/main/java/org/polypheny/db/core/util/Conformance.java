...
package org.polypheny.db.core.util;
...
public interface Conformance {
     ...
     * is equivalent to<br>
     * {@code SELECT * FROM emp CROSS JOIN LATERAL TABLE(promote(empno)}</li>
     *
     * <li>{@code SELECT * FROM emp OUTER APPLY TABLE(promote(empno)}<br>
     * is equivalent to<br>
     * {@code SELECT * FROM emp LEFT JOIN LATERAL TABLE(promote(empno)} ON true</li>
     * </ul>
     *
     * Among the built-in conformance levels, true in
     * {@link ConformanceEnum#BABEL},
     ...
     *
     * If {@code allowNiladicParentheses}, the following syntax is also valid:
     *
     * <blockquote><code>VALUES CURRENT_DATE()</code></blockquote>
     *
     ...
     *
     * The input columns have types {@code CHAR(5)} and {@code CHAR(3)}, and we need a result type that is large enough for both:
     *
     * <ul>
     * <li>Under strict SQL:2003 behavior, its column has type {@code CHAR(5)}, and the value in the second row will have trailing spaces.</li>
     * <li>With lenient behavior, its column has type {@code VARCHAR(5)}, and the values have no trailing spaces.</li>
     * </ul>
     *
     * Among the built-in conformance levels, true in
     * {@link ConformanceEnum#PRAGMATIC_99},
