...
package org.polypheny.db.core.rel;
...
/**
 * RelDecorrelator replaces all correlated expressions (corExp) in a relational expression (RelNode) tree with non-correlated expressions that are produced from joining the RelNode that produces the
 * corExp with the RelNode that references it.
 *
 * TODO:
 * <ul>
 * <li>replace {@code CorelMap} constructor parameter with a RelNode</li>
...
public class RelDecorrelator implements ReflectiveVisitor {
    ...
    public static RelNode decorrelateQuery( RelNode rootRel, RelBuilder relBuilder ) {
        ...
        final RelDecorrelator decorrelator = new RelDecorrelator( corelMap, cluster.getPlanner().getContext(), relBuilder );
    ...
    private RelNode createValueGenerator( Iterable<CorRef> correlations, int valueGenFieldOffset, SortedMap<CorDef, Integer> corDefOutputs ) {
        ...
        for ( CorRef corRef : correlations ) {
            // The first input of a Correlate is always the rel defining the correlated variables.
    ...
    private Frame decorrelateInputWithValueGenerator( RelNode rel, Frame frame ) {
        ...
        RelNode join = LogicalJoin.create( frame.r, valueGen, relBuilder.literal( true ), ImmutableSet.of(), JoinRelType.INNER );
    ...
    public Frame decorrelateRel( LogicalCorrelate rel ) {
        ...
        // Join all the correlated variables produced by this correlator rel with the values generated and propagated from the right input
        ...
        RelNode newJoin = LogicalJoin.create( leftFrame.r, rightFrame.r, condition, ImmutableSet.of(), rel.getJoinType().toJoinType() );
    ...
    private class RemoveCorrelationRexShuttle extends RexShuttle {
        ...
        public RexNode visitInputRef( RexInputRef inputRef ) {
            if ( currentRel instanceof LogicalCorrelate ) {
                // If this rel references corVar and now it needs to be rewritten it must have been pulled above the Correlate replace the input ref to account for the LHS of the
                // Correlate
    ...
    private static final class RemoveSingleAggregateRule extends RelOptRule {
        ...
        public void onMatch( RelOptRuleCall call ) {
            ...
            final RelDataType type = relBuilder.getTypeFactory().createTypeWithNullability( projExprs.get( 0 ).getType(), true );
    ...
    private final class AdjustProjectForCountAggregateRule extends RelOptRule {
        ...
        AdjustProjectForCountAggregateRule( boolean flavor, RelBuilderFactory relBuilderFactory ) {
            super(
                    flavor
                            ? operand( LogicalCorrelate.class, operand( RelNode.class, any() ), operand( LogicalProject.class, operand( LogicalAggregate.class, any() ) ) )
                            : operand( LogicalCorrelate.class, operand( RelNode.class, any() ), operand( LogicalAggregate.class, any() ) ),
                    relBuilderFactory,
                    null );
            this.flavor = flavor;
        }
     ...
     *
     * For instance, if a RelNode references emp.name multiple times, it would result in multiple {@code CorRef} objects that differ just in {@link CorRef#uniqueKey}.
     */
     ...
     * <ol>
     * <li>{@link #mapRefRelToCorRef} maps a {@link RelNode} to the correlated variables it references;</li>
    ...
    /**
     * Builds a {@link RelDecorrelator.CorelMap}.
     */
