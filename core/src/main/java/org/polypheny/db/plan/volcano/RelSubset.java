...
public class RelSubset extends AbstractRelNode {
     ...
     * <li>After creation, {@code best} and {@code bestCost} are maintained incrementally by {@link #propagateCostImprovements0} and {@link RelSet#mergeWith(VolcanoPlanner, RelSet)}.</li>
     * </ol>
     */
    ...
    void add( RelNode rel ) {
        ...
        // If this isn't the first rel in the set, it must have compatible row type.
        if ( set.rel != null ) {
    ...
    void propagateCostImprovements0( VolcanoPlanner planner, RelMetadataQuery mq,
        ...
        if ( !activeSet.add( this ) ) {
            // This subset is already in the chain being propagated to. This means that the graph is cyclic, and therefore the cost of this relational expression - not this subset - must be infinite.
    ...
    /**
     * Visitor which walks over a tree of {@link RelSet}s, replacing each node with the cheapest implementation of the expression.
     */
