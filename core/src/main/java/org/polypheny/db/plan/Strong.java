 ...
 * <li>{@code p1 AND p2} is strong in [p1, p2] (definitely null if either p1 is null or p2 is null)
 * <li>{@code p1 OR p2} is strong if p1 and p2 are strong
 * <li>{@code c1 = 1 OR c2 IS NULL} is strong in [c1] (definitely null if c1 is null)
 * </ul>
 */
public class Strong {
     ...
     *
     * The answer is based on calls to {@link #isNull} for its constituent expressions, and you may override methods to test hypotheses such as "if {@code x} is null, is {@code x + y} null?
     */
    public boolean isNull( RexNode node ) {
        ...
        switch ( node.getKind() ) {
            ...
            // We can only guarantee AND to return NULL if both inputs are NULL  (similar for OR) AND(NULL, FALSE) = FALSE
            case AND:
            case OR:
            case COALESCE:
