 ...
 * </ul>
 *
 * Note that the predicate from the left input appears in {@code rightInferredPredicates}. Predicates from several sources appear in {@code pulledUpPredicates}.
 */
public class AlgOptPredicateList {

    private static final ImmutableList<RexNode> EMPTY_LIST = ImmutableList.of();
    public static final AlgOptPredicateList EMPTY = new AlgOptPredicateList( EMPTY_LIST, EMPTY_LIST, EMPTY_LIST, ImmutableMap.of() );
    ...
    /**
     * A map of each (e, constant) pair that occurs within {@link #pulledUpPredicates}.
     */
    public final ImmutableMap<RexNode, RexNode> constantMap;


    private AlgOptPredicateList( ImmutableList<RexNode> pulledUpPredicates, ImmutableList<RexNode> leftInferredPredicates, ImmutableList<RexNode> rightInferredPredicates, ImmutableMap<RexNode, RexNode> constantMap ) {
     ...
     *
     * @param pulledUpPredicates Predicates that apply to the rows returned by the relational expression
     */
    public static AlgOptPredicateList of( RexBuilder rexBuilder, Iterable<RexNode> pulledUpPredicates ) {
     ...
     * @param pulledUpPredicates Predicates that apply to the rows returned by the relational expression
     * @param leftInferredPredicates Predicates that were inferred from the right input
     * @param rightInferredPredicates Predicates that were inferred from the left input
     */
    public static AlgOptPredicateList of( RexBuilder rexBuilder, Iterable<RexNode> pulledUpPredicates, Iterable<RexNode> leftInferredPredicates, Iterable<RexNode> rightInferredPredicates ) {
        ...
        return new AlgOptPredicateList( pulledUpPredicatesList, leftInferredPredicateList, rightInferredPredicatesList, constantMap );
    }


    public AlgOptPredicateList union( RexBuilder rexBuilder, AlgOptPredicateList list ) {
        if ( this == EMPTY ) {
            return list;
        } else if ( list == EMPTY ) {
            return this;
        } else {
            return AlgOptPredicateList.of(
                    rexBuilder,
    ...
    public AlgOptPredicateList shift( RexBuilder rexBuilder, int offset ) {
        return AlgOptPredicateList.of(
                rexBuilder,
