...
public class LoptSemiJoinOptimizer {
     ...
     */
    private AlgNode[] chosenSemiJoins;

    /**
     * Associates potential semijoins with each fact table factor. The first parameter in the map corresponds to the fact table. The second corresponds to the dimension table and a SemiJoin that captures all
    ...
    public LoptSemiJoinOptimizer( AlgMetadataQuery mq, LoptMultiJoin multiJoin, RexBuilder rexBuilder ) {
        // There are no semijoins yet, so initialize to the original factors
        this.mq = mq;
        int nJoinFactors = multiJoin.getNumJoinFactors();
        chosenSemiJoins = new AlgNode[nJoinFactors];
    ...
    /**
     * Determines all possible semijoins that can be used by dimension tables to filter fact tables. Constructs SemiJoinRels corresponding to potential dimension table filters and stores them in the member field
     * "possibleSemiJoins".
     *
    ...
    public void makePossibleSemiJoins( LoptMultiJoin multiJoin ) {
        ...
        for ( int factIdx = 0; factIdx < nJoinFactors; factIdx++ ) {
            ...
            // Loop over all filters and find equality filters that reference this factor and one other factor
            for ( RexNode joinFilter : multiJoin.getJoinFilters() ) {
                ...
                // If either the fact or dimension table is null generating, we cannot use semijoins
                if ( multiJoin.isNullGenerating( factIdx ) || multiJoin.isNullGenerating( dimIdx ) ) {
                    continue;
                }

                // if we've already matched against this dimension factor, then add the filter to the list associated with that dimension factor; otherwise, create a new entry
            ...
            // If there are potential dimension filters, determine if there are appropriate indexes
            ...
            for ( Integer dimIdx : dimIdxes ) {
                ...
                if ( joinFilters != null ) {
                    ...
                    // If an index is available, keep track of it as a possible semijoin
                    if ( semiJoin != null ) {
     ...
     * @return index of corresponding dimension table if the filter is appropriate; otherwise -1 is returned
     */
    private int isSuitableFilter( LoptMultiJoin multiJoin, RexNode joinFilter, int factIdx ) {
        // Ignore non-equality filters where the operands are not RexInputRefs
    ...
    private SemiJoin findSemiJoinIndexByCost( LoptMultiJoin multiJoin, List<RexNode> joinFilters, int factIdx, int dimIdx ) {
        ...
        // If necessary, truncate the keys to reflect the ones that match the index and remove the corresponding, unnecessary filters from the condition; note that we don't save the actual keys here because
        // later when the semijoin is pushed past other RelNodes, the keys will be converted
     ...
     * @param leftIdx index of the join factor corresponding to the LHS of the semijoin,
     * @param rightIdx index of the join factor corresponding to the RHS of the semijoin
    ...
    private RexNode adjustSemiJoinCondition( LoptMultiJoin multiJoin, int leftAdjustment, RexNode semiJoinCondition, int leftIdx, int rightIdx ) {
        // Adjust the semijoin condition to reflect the fact that the RHS is now the second factor into the semijoin and the LHS is the first
        ...
        if ( (leftAdjustment != 0) || (rightAdjustment != 0) ) {
            ...
            return semiJoinCondition.accept( new AlgOptUtil.RexInputConverter( rexBuilder, multiJoin.getMultiJoinFields(), adjustments ) );
        }
    ...
    /**
     * Validates the candidate semijoin keys corresponding to the fact table. Ensure the keys all originate from the same underlying table, and they all correspond to simple column references.
     * If unsuitable keys are found, they're removed from the key list and a new list corresponding to the remaining valid keys is returned.
     *
     * @param factRel fact table AlgNode
    ...
    private LcsEntity validateKeys( AlgNode factRel, List<Integer> leftKeys, List<Integer> rightKeys, List<Integer> actualLeftKeys ) {
        ...
        while ( keyIter.hasNext() ) {
            ...
            final AlgColumnOrigin colOrigin = mq.getColumnOrigin( factRel, keyIter.next() );
            ...
            if ( (colOrigin == null) || LucidDbSpecialOperators.isLcsRidColumnId( colOrigin.getOriginColumnOrdinal() ) ) {
                ...
                if ( theTable == null ) {
                    ...
                    // The tables must match because the column has a simple origin
        ...
        // if all keys have been removed, then we don't have any valid semijoin keys
    ...
    /**
     * Removes from an expression any sub-expressions that reference key values that aren't contained in a key list passed in. The keys represent join keys on one side of a join. The subexpressions
     * are all assumed to be of the form "tab1.col1 = tab2.col2".
     *
     * @param keys join keys from one side of the join
     * @param nFields number of fields in the side of the join for which the keys correspond
     * @param condition original expression
     * @return modified expression with filters that don't reference specified keys removed
     */
    private RexNode removeExtraFilters( List<Integer> keys, int nFields, RexNode condition ) {
        ...
        if ( condition.isA( Kind.AND ) ) {
            ...
            return rexBuilder.makeCall( OperatorRegistry.get( OperatorName.AND ), left, right );
        }
    ...
    public boolean chooseBestSemiJoin( LoptMultiJoin multiJoin ) {
        ...
        // Loop through the factors in sort order, treating the factor as a fact table; analyze the possible semijoins associated with that fact table.
        for ( int i = 0; i < nJoinFactors; i++ ) {
            Integer factIdx = sortedFactors.get( i );
            AlgNode factRel = chosenSemiJoins[factIdx];
            ...
            // Loop through each dimension table associated with the current fact table and analyze the ones that have semijoins with this fact table
            ...
            for ( Integer dimIdx : dimIdxes ) {
                ...
                // Keep track of the dimension table that has the best score for filtering this fact table
            ...
            // If a suitable dimension table has been found, associate it with the fact table in the chosenSemiJoins array; also remove the entry from possibleSemiJoins so we won't chose it again;
            // note that we create the SemiJoin using the chosen semijoins already created for each factor so any chaining of filters will be accounted for.
            if ( bestDimIdx != -1 ) {
                ...
                // Determine if the dimension table doesn't need to be joined as a result of this semijoin
                removeJoin( multiJoin, chosenSemiJoin, factIdx, bestDimIdx );

                removePossibleSemiJoin( possibleDimensions, factIdx, bestDimIdx );

                // Need to also remove the semijoin from the possible semijoins associated with this dimension table, as the semijoin can only be used to filter one table, not both.
                removePossibleSemiJoin( possibleSemiJoins.get( bestDimIdx ), bestDimIdx, factIdx );
                return true;
            }

            // Continue searching on the next fact table if we couldn't find a semijoin for the current fact table
        }
     ...
     * @return computed score of applying the dimension table filters on the fact table
     */
    private double computeScore( AlgNode factRel, AlgNode dimRel, SemiJoin semiJoin ) {
        // Estimate savings as a result of applying semijoin filter on fact table.  As a heuristic, the selectivity of the semijoin needs to be less than half.
        // There may be instances where an even smaller selectivity value is required because of the overhead of index lookups on a very large fact table.
        // Half was chosen as a middle ground based on testing that was done with a large data set.
        final ImmutableBitSet dimCols = ImmutableBitSet.of( semiJoin.getRightKeys() );
        final double selectivity = AlgMdUtil.computeSemiJoinSelectivity( mq, factRel, dimRel, semiJoin );
        if ( selectivity > .5 ) {
            return 0;
        }

        final AlgOptCost factCost = mq.getCumulativeCost( factRel );
        ...
        // Additional savings if the dimension columns are unique.  We can ignore nulls since they will be filtered out by the semijoin.
        boolean uniq = AlgMdUtil.areColumnsDefinitelyUniqueWhenNullsFiltered( mq, dimRel, dimCols );
        if ( uniq ) {
            savings *= 2.0;
        }
    ...
    /**
     * Determines whether a join of the dimension table in a semijoin can be removed. It can be if the dimension keys are unique and the only fields referenced from the dimension table are its semijoin keys.
     * The semijoin keys can be mapped to the corresponding keys from the fact table (because of the equality condition associated with the semijoin keys). Therefore, that's why the dimension table can be removed even though those fields
    ...
    private void removeJoin( LoptMultiJoin multiJoin, SemiJoin semiJoin, int factIdx, int dimIdx ) {
        // If the dimension can be removed because of another semijoin, then no need to proceed any further
        if ( multiJoin.getJoinRemovalFactor( dimIdx ) != null ) {
            return;
        }

        // Check if the semijoin keys corresponding to the dimension table are unique.  The semijoin will filter out the nulls.
        final ImmutableBitSet dimKeys = ImmutableBitSet.of( semiJoin.getRightKeys() );
        final AlgNode dimRel = multiJoin.getJoinFactor( dimIdx );
        if ( !AlgMdUtil.areColumnsDefinitelyUniqueWhenNullsFiltered( mq, dimRel, dimKeys ) ) {
            return;
        }

        // Check that the only fields referenced from the dimension table in either its projection or join conditions are the dimension keys
        ...
        // Criteria met; keep track of the fact table and the semijoin that allow the join of this dimension table to be removed
        multiJoin.setJoinRemovalFactor( dimIdx, factIdx );
        multiJoin.setJoinRemovalSemiJoin( dimIdx, semiJoin );

        // If the dimension table doesn't reference anything in its projection and the only fields referenced in its joins are the dimension keys
        // of this semijoin, then we can decrement the join reference counts corresponding to the fact table's semijoin keys, since the
     ...
     *
     * @param possibleDimensions possible dimension tables associated with the fact table
    ...
    private void removePossibleSemiJoin( Map<Integer, SemiJoin> possibleDimensions, Integer factIdx, Integer dimIdx ) {
        // Dimension table may not have a corresponding semijoin if it wasn't indexable
        if ( possibleDimensions == null ) {
            return;
        }
     ...
     * @return optimal semijoin for the specified factor; may be the factor itself if semijoins are not chosen for the factor
     */
    public AlgNode getChosenSemiJoin( int factIdx ) {
    ...
    private static class LcsIndexOptimizer {
        ...
        public FemLocalIndex findSemiJoinIndexByCost( AlgNode dimRel, List<Integer> actualLeftKeys, List<Integer> rightKeys, List<Integer> bestKeyOrder ) {
            return null;
        }
