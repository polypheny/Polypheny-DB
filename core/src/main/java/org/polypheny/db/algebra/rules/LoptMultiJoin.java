...
public class LoptMultiJoin {
     ...
     */
    private final ImmutableList<AlgNode> joinFactors;
    ...
    /**
     * Map containing reference counts of the fields referenced in join conditions for each join factor. If a field is only required for a semijoin, then it is removed from the reference count.
     * (Hence the need for reference counts instead of simply a bitmap.) The map is indexed by the factor number.
     */
     ...
     * Type factory
     */
    final AlgDataTypeFactory factory;

    /**
     * Indicates for each factor whether its join can be removed because it is the dimension table in a semijoin. If it can be, the entry indicates the factor id of the fact table
     * (corresponding to the dimension table) in the semijoin that allows the factor to be removed. If the factor cannot be removed, the entry corresponding to the factor is null.
     */
     ...
     * The map is keyed by either factor in the self join.
     */
    ...
    public LoptMultiJoin( MultiJoin multiJoin ) {
        ...
        joinFilters = Lists.newArrayList( AlgOptUtil.conjunctions( multiJoin.getJoinFilter() ) );

        allJoinFilters = new ArrayList<>( joinFilters );
        List<RexNode> outerJoinFilters = multiJoin.getOuterJoinConditions();
        for ( int i = 0; i < nJoinFactors; i++ ) {
            allJoinFilters.addAll( AlgOptUtil.conjunctions( outerJoinFilters.get( i ) ) );
        }
        ...
        // Extract outer join information from the join factors, including the type of outer join and the factors that a null-generating factor is dependent upon.
        ...
        for ( int i = 0; i < nJoinFactors; i++ ) {
            if ( outerJoinConds.get( i ) != null ) {
                // set a bitmap containing the factors referenced in the ON condition of the outer join; mask off the factor corresponding to the factor itself
     ...
     */
    public AlgNode getJoinFactor( int factIdx ) {
     ...
     * @return bitmap corresponding to the factors referenced by the specified factor in the various join filters that correspond to comparisons
     */
    ...
    /**
     * @param factIdx factor for which information will be returned
     * @return whether or not the factor corresponds to a null-generating factor in a left or right outer join
     */
    public boolean isNullGenerating( int factIdx ) {
        return joinTypes.get( factIdx ) != JoinAlgType.INNER;
    }


    /**
     * @param factIdx factor for which information will be returned
     * @return bitmap containing the factors that a null generating factor is dependent upon, if the factor is null generating in a left or right outer join; otherwise null is returned
     */
     ...
     * @return the factor id of the fact table corresponding to a dimension table in a semijoin, in the case where the join with the dimension table can be removed
     */
    ...
    private ImmutableBitSet fieldBitmap( RexNode joinFilter ) {
        final AlgOptUtil.InputFinder inputFinder = new AlgOptUtil.InputFinder();
    ...
    private void setJoinFilterRefs() {
        ...
        while ( filterIter.hasNext() ) {
            ...
            // ignore the literal filter; if necessary, we'll add it back later
    ...
    /**
     * Sets weighting for each combination of factors, depending on which join filters reference which factors. Greater weight is given to equality conditions.
     * Also, sets bitmaps indicating which factors are referenced by each factor within join filters that are comparisons.
     */
    public void setFactorWeights() {
        ...
        for ( RexNode joinFilter : allJoinFilters ) {
            ...
            // OR the factors referenced in this join filter into the bitmaps corresponding to each of the factors; however, exclude the bit corresponding to the factor itself
            ...
            if ( factorRefs.cardinality() == 2 ) {
                ...
                // filter contains only two factor references, one on each side of the operator
                int weight;
     ...
     * @return true if the factor corresponds to the null generating factor in an outer join that can be removed
     */
    ...
    public void addRemovableSelfJoinPair( int factor1, int factor2 ) {
        ...
        // Put the factor with more fields on the left so it will be preserved after the self-join is removed.
        ...
        // Compute a column mapping such that if a column from the right factor is also referenced in the left factor, we will map the right reference to the left to avoid redundant references.
        final Map<Integer, Integer> columnMapping = new HashMap<>();

        // First, locate the originating column for all simple column references in the left factor.
     ...
     * @return the offset of the corresponding column in the left factor, if such a column mapping exists; otherwise, null is returned
     */
