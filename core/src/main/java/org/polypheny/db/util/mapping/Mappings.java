...
public abstract class Mappings {
     ...
     *
     * For example, {@code createIdentity(2)} returns the mapping {0:0, 1:1, 2:2}.
     *
     ...
     *
     * {@code divide(A, B)} returns a mapping C such that B . C (the mapping B followed by the mapping C) is equivalent to A.
     *
    ...
    public static ImmutableList<ImmutableBitSet> apply2( final Mapping mapping, Iterable<ImmutableBitSet> bitSets ) {
        return ImmutableList.copyOf( Iterables.transform( bitSets, input1 -> apply( mapping, input1 ) ) );
    }
    ...
    /**
     * Returns a mapping as a list such that {@code list.get(source)} is {@code mapping.getTarget(source)} and {@code list.size()} is {@code mapping.getSourceCount()}.
     *
     ...
     *
     * For example, <pre>append({0:0, 1:1}, {0:0, 1:1, 2:2})</pre> yields <pre>{0:0, 1:1, 2:2, 3:3, 4:4}</pre>.
     *
     * @see #merge
     */
     ...
     *
     * For example, <code>merge({0:0, 1:1}, {2:2, 3:3, 4:4})</code> yields <code>{0:0, 1:1, 2:2, 3:3, 4:4}</code>.
     * <code>merge({0:0, 1:1}, {1:2, 2:3})</code> throws, because there are two entries with source=1.
     */
     ...
     *
     * For example, given {@code mapping} with sourceCount=2, targetCount=8, and (source, target) entries {[0: 5], [1: 7]}, offsetSource(mapping, 3) returns a mapping with sourceCount=5, targetCount=8,
     * and (source, target) entries {[3: 5], [4: 7]}.
     *
     * @param mapping Input mapping
     * @param offset Offset to be applied to each source
     * @param sourceCount New source count; must be at least {@code mapping}'s source count plus {@code offset}
     * @return Shifted mapping
     */
     ...
     *
     * For example, given {@code mapping} with sourceCount=2, targetCount=8, and (source, target) entries {[0: 5], [1: 7]}, offsetTarget(mapping, 3) returns a mapping with sourceCount=2, targetCount=11,
     * and (source, target) entries {[0: 8], [1: 10]}.
     *
     * @param mapping Input mapping
     * @param offset Offset to be applied to each target
     * @param targetCount New target count; must be at least {@code mapping}'s target count plus {@code offset}
     * @return Shifted mapping
     */
    ...
    /**
     * Inverts an {@link java.lang.Iterable} over {@link IntPair}s.
     */
    ...
    /**
     * Inverts an {@link java.util.Iterator} over {@link IntPair}s.
     */
     ...
     *
     * <ul>
     * <li>A target may not have a source.</li>
     * <li>May not be finite.</li>
     * </ul>
     */
     ...
     *
     * <ul>
     * <li>It has a finite number of sources and targets</li>
     * <li>Each target has exactly one source</li>
     * <li>Each source has at most one target</li>
     * </ul>
     *
     ...
     * <ul>
     * <li>It has a finite number of sources and targets</li>
     * <li>Each target has at most one source</li>
     * <li>Each source has exactly one target</li>
     * </ul>
     *
     * TODO: figure out which interfaces this should extend
     */
    ...
    /**
     * Iterator that yields the (source, target) values in a {@link FunctionMapping}.
     */
    ...
    public static class PartialMapping extends FiniteAbstractMapping implements Mapping, FunctionMapping, TargetMapping {
         ...
         * @param mappingType Mapping type; must not allow multiple sources per target or multiple targets per source
         */
        ...
        /**
         * Creates a partial mapping from a list. For example, <code>PartialMapping({1, 2, 4}, 6)</code> creates the mapping
         *
         * <table border="1">
         * <caption>Example</caption>
         * <tr>
     ...
     *
     * If you call {@link #set} on a target, the target's previous source will be lost.
     */
    ...
    /**
     * Source mapping that returns the same result as a parent {@link SourceMapping} except for specific overriding elements.
     */
    public static class OverridingSourceMapping extends AbstractMapping implements SourceMapping {
        ...
        @Override
        public boolean isIdentity() {
            // FIXME: It's possible that parent was not the identity but that this overriding fixed it.
    ...
    /**
     * Target mapping that returns the same result as a parent {@link TargetMapping} except for specific overriding elements.
     */
    public static class OverridingTargetMapping extends AbstractMapping implements TargetMapping {
        ...
        @Override
        public boolean isIdentity() {
            // FIXME: Possible that parent is not identity but this overriding fixes it.
    ...
    /**
     * Implementation of {@link Mapping} where a source can have at most one target, and a target can have any number of sources. The source count must be finite, but the target count may be infinite.
     *
