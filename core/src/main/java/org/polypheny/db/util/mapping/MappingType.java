...
/**
 * Describes the type of a mapping, from the most general {@link #MULTI_FUNCTION} (every element in the source and target domain can participate in many mappings) to the most retricted {@link #BIJECTION} (every
 * element in the source and target domain must be paired with precisely one element in the other domain).
 *
 * Some common types:
 *
 * <ul>
 * <li>A surjection is a mapping if every target has at least one source; also known as an 'onto' mapping.</li>
 * <li>A mapping is a partial function if every source has at most one target.</li>
 * <li>A mapping is a function if every source has precisely one target.</li>
 * <li>An injection is a mapping where a target has at most one source; also somewhat confusingly known as a 'one-to-one' mapping.</li>
 * <li>A bijection is a mapping which is both an injection and a surjection. Every source has precisely one target, and vice versa.</li>
 * </ul>
 *
 * Once you know what type of mapping you want, call {@link Mappings#create(MappingType, int, int)} to create an efficient implementation of that mapping.
 */
public enum MappingType {
     ...
     *
     * Obeys the constaints {@link MappingType#isMandatorySource()}, {@link MappingType#isSingleSource()}.
     *
     * Similar types:
     *
     * <ul>
     * <li>{@link #INVERSE_SURJECTION} is stronger (a source may not have multiple targets);</li>
     * <li>{@link #INVERSE_PARTIAL_FUNCTION} is weaker (a target may have 0 or 1 sources).</li>
     * </ul>
     */
     ...
     *
     * For example, a {@link #BIJECTION} is a {@link #FUNCTION}, but not every {link #Function} is a {@link #BIJECTION}.
     */
