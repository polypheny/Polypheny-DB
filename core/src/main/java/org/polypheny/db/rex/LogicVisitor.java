...
public class LogicVisitor implements RexBiVisitor<Logic, Logic> {
     ...
     *
     * Chooses a logic that is safe (that is, gives the right answer) with the fewest possibilities (that is, we prefer one that returns [true as true, false as false, unknown as false] over one that
    ...
    public static Logic find( Logic logic, List<RexNode> nodes, RexNode seek ) {
        ...
        // Convert FALSE (which can only exist within LogicVisitor) to UNKNOWN_AS_TRUE.
