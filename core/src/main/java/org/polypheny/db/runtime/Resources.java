...
public class Resources {
    ...
    /**
     * Returns the preferred locale of the current thread, or the default locale if the current thread has not called {@link #setThreadLocale}.
     *
     * @return Locale
     */
    ...
    /**
     * Returns the preferred locale of the current thread, or null if the thread has not called {@link #setThreadLocale}.
     *
     * @return Locale
     */
     ...
     * @return Instance of the interface that can be used to instantiate resources
    ...
    private static <T> T create( final String base, final PropertyAccessor accessor, Class<T> clazz ) {
        ...
        return (T) Proxy.newProxyInstance( clazz.getClassLoader(),
                ...
                new InvocationHandler() {
                    ...
                    private Object create( Method method, Object[] args ) throws NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
                        ...
                        try {
                            if ( Inst.class.isAssignableFrom( returnType ) ) {
                                final Constructor<?> constructor = returnType.getConstructor( String.class, Locale.class, Method.class, Object[].class );
                                ...
                                final Constructor<?> constructor = returnType.getConstructor( PropertyAccessor.class, Method.class );
    ...
    public static class Inst extends Element {
                ...
                switch ( validation ) {
                    ...
                    case EVEN_QUOTES:
                        ...
                        if ( countQuotesIn( message ) % 2 == 1 ) {
                            throw new AssertionError( "resource '" + method.getName() + "' should have even number of quotes" );
                        }
                        break;
                    case MESSAGE_MATCH:
                    ...
                    case ARGUMENT_MATCH:
                        ...
                        if ( !types.equals( parameterTypeList ) ) {
                            throw new AssertionError( "type mismatch in method '" + method.getName() + "' between message format elements " + types + " and method parameters " + parameterTypeList );
                        }
                        break;
                }
            }
        ...
        public String raw() {
            try {
                ...
                // Resource is not in the bundle. (It is probably missing from the .properties file.) Fall back to the base message.
        ...
        public Map<String, String> getProperties() {
            // At present, annotations allow at most one property per resource. We could design new annotations if any resource needed more.
    ...
    /**
     * Sub-class of {@link Inst} that can throw an exception. Requires caused by exception.
     */
    public static class ExInstWithCause<T extends Exception> extends Inst {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        // Ignore nsmString to encourage users to have (String, Throwable) constructors.
        ...
        protected void validateException( Callable<Exception> exSupplier ) {
            ...
            try {
                ...
                // Just in case we get exception somehow, we will rethrow it as a part of AssertionError below.
                cause = e;
            }
            if ( cause != null ) {
                AssertionError assertionError = new AssertionError( "error instantiating exception for resource '" + method.getName() + "'" );
    ...
    /**
     * <code>ShadowResourceBundle</code> is an abstract base class for {@link ResourceBundle} classes which are backed by a properties file.
     * When the class is created, it loads a properties file with the same name as the class.
     *
     * In the standard scheme (see {@link ResourceBundle}), if you call <code>{@link ResourceBundle#getBundle}("foo.MyResource")</code>, it first looks for a class called <code>foo.MyResource</code>,
     * then looks for a file called <code>foo/MyResource.properties</code>. If it finds the file, it creates a {@link PropertyResourceBundle} and loads the class. The problem is if you want to load
     * the <code>.properties</code> file into a dedicated class; <code>ShadowResourceBundle</code> helps with this case.
     *
     * You should create a class as follows:<blockquote>
     *
     * <pre>package foo;
     ...
     * </blockquote>
     *
     * Then when you call {@link ResourceBundle#getBundle ResourceBundle.getBundle("foo.MyResource")}, it will find the class before the properties file, but still automatically load the
     * properties file based upon the name of the class.
     */
    public abstract static class ShadowResourceBundle extends ResourceBundle {
        ...
        /**
         * Creates a <code>ShadowResourceBundle</code>, and reads resources from a <code>.properties</code> file with the same name as the current class.
         * For example, if the class is called <code>foo.MyResource_en_US</code>, reads from <code>foo/MyResource_en_US.properties</code>, then <code>foo/MyResource_en.properties</code>,
         * then <code>foo/MyResource.properties</code>.
         *
        ...
        protected ShadowResourceBundle() throws IOException {
            ...
            stream.close();
            // Now load properties files for parent locales, which we deduce from the names of our super-class, and its super-class.
            while ( true ) {
        ...
        /**
         * Returns the instance of the <code>baseName</code> resource bundle for the given locale.
         *
         * This method should be called from a derived class, with the proper casting:<blockquote>
         *
    ...
    enum BuiltinMethod {
         ...
         * Includes in its search inherited methods and methods with wider argument types.
         *
    ...
    /**
     * Implementation of {@link PropertyAccessor} that reads from a {@link Properties}.
     */
