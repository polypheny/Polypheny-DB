...
class DruidConnectionImpl implements DruidConnection {
    ...
    static {
        ...
        UTC_TIMESTAMP_FORMAT = new SimpleDateFormat( "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale.ROOT );
    ...
    private void parseFieldForName( List<String> fieldNames, List<ColumnMetaData.Rep> fieldTypes, int posTimestampField, Row.RowBuilder rowBuilder, JsonParser parser, String fieldName ) throws IOException {
        ...
        if ( isTimestampColumn || ColumnMetaData.Rep.JAVA_SQL_TIMESTAMP == type ) {
            ...
            if ( token == JsonToken.VALUE_NUMBER_INT ) {
                ...
                // We don't have any way to figure out the format of time upfront since we only have org.apache.calcite.avatica.ColumnMetaData.Rep.JAVA_SQL_TIMESTAMP as type to represent
                // both timestamp and timestamp with local timezone. Logic where type is inferred can be found at DruidQuery.DruidQueryNode.getPrimitive(). Thus need to guess via try and catch
    ...
    private Long extractTimestampField( JsonParser parser ) throws IOException {
        ...
        if ( !parser.getCurrentName().equals( DEFAULT_RESPONSE_TIMESTAMP_COLUMN ) ) {
            throw new RuntimeException( "expected field " + DEFAULT_RESPONSE_TIMESTAMP_COLUMN + ", got " + parser.getCurrentName() );
        }
        parser.nextToken();
        try {
            final Date parse;
            ...
            for ( JsonSegmentMetadata o : list ) {
                ...
                if ( o.aggregators != null ) {
                    for ( Map.Entry<String, JsonAggregator> entry : o.aggregators.entrySet() ) {
                        ...
                        if ( type.isComplex() ) {
                            // Each complex type will get their own alias, equal to their actual name. Maybe we should have some smart string replacement strategies to make the column names more natural.
    ...
    Set<String> tableNames() {
        ...
        try ( InputStream in0 = post( url, data, requestHeaders, 10000, 1800000 );
            ...
            final CollectionType listType = mapper.getTypeFactory().constructCollectionType( List.class, String.class );
    ...
    private InputStream traceResponse( InputStream in ) {
        if ( RuntimeConfig.DEBUG.getBoolean() ) {
            try {
                final byte[] bytes = AvaticaUtils.readFullyToBytes( in );
                in.close();
                System.out.println( "Response: " + new String( bytes, StandardCharsets.UTF_8 ) ); // CHECKSTYLE: IGNORE 0
                in = new ByteArrayInputStream( bytes );
            } catch ( IOException e ) {
                throw new RuntimeException( e );
            }
    ...
    /**
     * An {@link Enumerator} that gets its rows from a {@link BlockingQueue}. There are other fields to signal errors and end-of-data.
     *
