...
public class DruidDateTimeUtils {
    ...
    /**
     * Generates a list of {@link Interval}s equivalent to a given expression. Assumes that all the predicates in the input reference a single column: the timestamp column.
     */
    @Nullable
    ...
    protected static List<Range<Long>> extractRanges( RexNode node, boolean withNot ) {
        switch ( node.getKind() ) {
            ...
            case AND: {
                ...
                for ( RexNode child : call.getOperands() ) {
                    List<Range<Long>> extractedRanges = extractRanges( child, false );
    ...
    protected static List<Range<Long>> leafToRanges( RexCall call, boolean withNot ) {
        switch ( call.getKind() ) {
            ...
            case BETWEEN: {
                final Long value1;
                final Long value2;
                if ( literalValue( call.getOperands().get( 2 ) ) != null && literalValue( call.getOperands().get( 3 ) ) != null ) {
    ...
    protected static Long literalValue( RexNode node ) {
        switch ( node.getKind() ) {
            case LITERAL:
                ...
                break;
            case CAST:
                // Normally all CASTs are eliminated by now by constant reduction. But when HiveExecutor is used there may be a cast that changes only nullability, from TIMESTAMP NOT NULL literal to TIMESTAMP literal.
    ...
    /**
     * Infers granularity from a time unit. It supports {@code FLOOR(<time> TO <timeunit>)} and {@code EXTRACT(<timeunit> FROM <time>)}. Returns null if it cannot be inferred.
     *
    ...
    /**
     * Translates Polypheny-DB TimeUnitRange to Druid {@link Granularity}
     *
     * @param timeUnit Polypheny-DB Time unit to convert
