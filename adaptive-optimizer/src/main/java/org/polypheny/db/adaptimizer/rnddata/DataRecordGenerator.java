...
import org.polypheny.db.adaptimizer.SeedUtil;
...
public class DataRecordGenerator {
    ...
    public List<Long> columnsOf( Long table ) {
        return AdaptiveOptimizerImpl.getCatalog().getTable( table ).columnIds;
    }

    public List<CatalogColumn> catalogColumnsOf( List<Long> columns ) {
        return columns.stream().map( AdaptiveOptimizerImpl.getCatalog()::getColumn ).collect( Collectors.toList() );
    }
    ...
    private RndDataException exceptType( ColumnOption option ) {
        return new RndDataException( String.format( "Type not implemented: %s", option.getType().getName()), new IllegalArgumentException("") );
    }


    private RndDataException exceptPk( ColumnOption option ) {
        return new RndDataException( String.format( "Primary key not allowed for %s", option.getType().getName() ), new IllegalArgumentException("") );
    }
