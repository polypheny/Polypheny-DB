...
public class DataGeneratorBuilder implements Builder<DataGenerator> {
    ...
    public DataGeneratorBuilder addCatalogTable( CatalogTable catalogTable, int size ) {
        catalogTable.columnIds.stream().map( ColumnOption::new ).forEach( o -> options.put( o.getCatalogColumn(), o ) );
        catalogTable.columnIds.forEach(c -> getOptions().get( c ).setType( catalog.getColumn( c ).type ) );
    ...
    public DataGeneratorBuilder addDateRangeOption( CatalogColumn column, String start, String end ) {
        try {
            ...
            throw new RndDataException( "Range could not be parsed", e );
        }
        return this;
    }
    ...
    private void updateOptionsWithKeys() {
        getSizes().keySet().forEach( table -> {
            AdaptiveOptimizerImpl.getCatalog().getForeignKeys( table ).forEach( k -> {
            ...
            for ( Long primaryKeyColumnId : AdaptiveOptimizerImpl.getCatalog().getPrimaryKey( AdaptiveOptimizerImpl.getCatalog().getTable( table ).primaryKey ).columnIds ) {
    ...
    private DataRecordGenerator getRecordGenerator() {
        ...
        return new DataRecordGenerator(
                ...
                new ArrayList<>( getOptions().keySet() ).stream().collect( Collectors.toMap( c -> c, c -> getSizes().get(
                        AdaptiveOptimizerImpl.getCatalog().getColumn( c ).tableId ) ) ),
